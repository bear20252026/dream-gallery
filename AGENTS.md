...
- **剧本对话链规矩(2026-09-10 容错改造)**:凡剧本台词(crash-site 叫醒词/scene2 画羊/scene3 转夜与回忆层)一律经 `ctx.openDialog` **带 `lock:true`**(互斥:上一条 lock 对话未关时忽略后来 lock,多系统 speakSeq 不再互相覆盖)与 **`speakerType`**(story-text.mjs who 常量的 `spk` 字段,经 `whoSpk()` 取;prince 金/pilot 蓝/sheep 粉/rose 红,[data-spk] CSS 换装);**链式对话必须带心跳守护**——`ctx.dialogOpen()` 每 2.6s 余量轮询一次,onDone 意外丢失时兜底推进,且每步 `spent` 幂等守卫防 onDone 与守护双推进。**禁止在调用方手写 onDone 循环**(scene2 finale 曾自造循环无类型无守护,已归并 speakSeq)。打断语义:非 lock openDialog 可打断 lock 链,被打断项的 onDone 由 gameshell-dialog 延后一拍补发(链自恢复)。验收:scripts/probe/b612-story-chain-probe.cjs(23 项,支持 BASE_URL 直测线上)。
