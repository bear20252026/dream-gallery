...
// 每次公开上传前/后调用(dk=上传者设备,name=文件名,size=字节)
// 返回 null=放行;返回 'rate'|'bigfile'|'volume'=调用方应硬拒(429);'storage' 只预警不拦单人
function abuseCheck(dk, name, size) {
  try {
    if (!gateData.uploads) return null;
    const now = Date.now();
    let block = null;
    // 1. 速率
