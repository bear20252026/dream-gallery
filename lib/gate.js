...
const {
  ...
  checkPass, recordVisit, isAutomated,
} = require('./store');
...
// 身份归并:vid Cookie → dk(UA) → UA 全文 → 精确匹配新建
...
function entryGate(req, res, pathname) {
  ...
  const a = id ? gateData.applicants[id] : null;
  if (a) {
...
function handleCollect(req, res) {
  readBody(req, obj => {
    ...
    if (!myId) myId = crypto.randomBytes(8).toString('hex');
    const me = gateData.applicants[myId] || {};
