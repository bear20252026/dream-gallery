...
const {
  ...
  parseBrand, deviceKey, findByDevice, signId,
...
function fpHash(req, fp) {
  ...
  return crypto.createHash('sha1').update((req.socket.remoteAddress || '') + '|' + stable).digest('hex').slice(0, 16);
}

function handleApply(req, res) {
  readBody(req, obj => {
    if (gateData.blockedIps.includes(req.socket.remoteAddress)) { sendJson(res, 403, { error: '很抱歉,您暂时无法参观' }); return; }
...
function approvalGate(req, res, pathname) {
  if (gateData.blockedIps.includes(req.socket.remoteAddress)) {
