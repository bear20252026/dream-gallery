...
// 阅卷成本闸(2026-09-23 落地,对齐 vision 的每设备日限):
// 每次交卷的问答都会真实调用 AI 通道 → 原实现无限流,费用可被刷(旧审计遗留 P0)。
// 每设备每天 20 次,与 vision 同档;超限时**不拦截交卷**,只跳过 AI 改用本地细则兜底,
// 保证访客体验不中断(本地细则封顶 13 分,AI 上限 19 分)。
const QUIZ_AI_DAILY_MAX = 20;
const QUIZ_TRACKS = {
...
// AI 阅卷:通道登记处单一源(lib/aichannels.js,2026-07-28 主人定小米首选);失败/未配置时自动回退本地细则
const { chatText } = require('./aichannels');
async function gradeQAWithAI(question, answer, req) {
  ...
  const r = await chatText({
    ...
    maxTokens: 600, temperature, timeoutMs: 90000,
  });
  if (!r) return null;
  try {
...
async function handleQuizSubmit(req, res) {
  readBody(req, async obj => {
    ...
    // 成本闸:每设备每天 20 次 AI 阅卷(2026-09-23);额度用尽或未带 req 时直接走本地细则
    let graded = null;
    if (req) {
      const day = new Date().toISOString().slice(0, 10);
      if (!gateData.quizAiQuota) gateData.quizAiQuota = {};
      const qk = quotaKey(req);
      const q = gateData.quizAiQuota[qk] || { day, n: 0 };
      if (q.day !== day) { q.day = day; q.n = 0; }
      if (q.n < QUIZ_AI_DAILY_MAX) {
        q.n++; gateData.quizAiQuota[qk] = q;
        if (Object.keys(gateData.quizAiQuota).length > 3000) capKeys(gateData.quizAiQuota, 2500);
        graded = await gradeQAWithAI(s.qa.q, qaText, req);
      }
    } else {
      graded = await gradeQAWithAI(s.qa.q, qaText, req);
    }
    const grading = graded || gradeQALocal(s.qa.q, qaText);
