...
const { tokenOk, tokenOkPage, handleAdminList, handleAdminDecide, handleAdminBulk, handleAdminQuiz } = require('./admin');
...
const { handleTts, handleTtsBatch, handleTtsAudio } = require('./tts');
const { handleTtsStats, handleAdminTtsStats, handleAdminTtsStatsClear } = require('./tts-stats');
...
const ROUTES = [
  // ===================== 协议文档在线编辑器 =====================
  { method: 'GET',  match: '/admin/docs',            auth: 'adminPage', fn: adminPage('docs.html') },
  ...
  // ===================== 主人后台页面 =====================
  { method: 'GET',  match: '/admin',                 auth: 'adminPage', fn: adminPage('admin.html') },
  ...
  { method: 'GET',  match: '/api/admin/tts-stats',           auth: 'token',  fn: (c) => handleAdminTtsStats(c.req, c.res) },
  { method: 'POST', match: '/api/admin/tts-stats/clear',     auth: 'token',  fn: (c) => handleAdminTtsStatsClear(c.req, c.res) },
  ...
  // 台词音频边缘可缓存出口(2026-09-26):.mp3 扩展名命中 CF 默认缓存清单,客户端复算键直拼 URL
  { method: 'GET',  match: /^\/tts-audio\/([0-9a-f]{20})\.mp3$/, auth: 'public', fn: (c) => handleTtsAudio(c.req, c.res, c.m[1]) },
  { method: 'POST', match: '/api/tts/batch',    auth: 'public', fn: (c) => handleTtsBatch(c.req, c.res) }, // 剧情预合成(2026-09-26)
  { method: 'POST', match: '/api/tts/stats',    auth: 'public', fn: (c) => handleTtsStats(c.req, c.res) }, // 台词播放追踪打点(2026-09-26)
...
function dispatch(req, res, pathname, query) {
  for (const r of ROUTES) {
    ...
    // 后台页面 bootstrap(2026-09-20 M3):query 打页始终可(TOKEN 必验),数据 API 仍走 tokenOk 严校
    if (r.auth === 'adminPage' && !tokenOkPage(req, query)) { sendJson(res, 401, { error: '未授权:需要 token' }); return true; }
    // write:配了 TOKEN 则强制鉴权;未配 TOKEN 时公网 Host 一律拒(2026-09-18 审计 P2#6),仅 localhost/回环放行
    if (r.auth === 'write') {
      if (TOKEN && !tokenOk(req, query)) { sendJson(res, 401, { error: '未授权:缺少或错误的 token' }); return true; }
      if (!TOKEN) {
        const host = String(req.headers.host || '');
        const isLocal = /^(localhost|127\.0\.0\.1)(:\d+)?$/.test(host);
        if (!isLocal) { sendJson(res, 401, { error: '未授权:未配置 TOKEN 的写接口仅限本地' }); return true; }
      }
    }
    r.fn(c);
    return true;
