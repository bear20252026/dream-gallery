...
// 响应: { ok, cached, queued } —— 前端 fire-and-forget,失败静默不影响页面。
  ...
  readBody(req, obj => {
    try {
      ...
      for (const it of items) {
        ...
        try { fs.mkdirSync(CACHE_DIR, { recursive: true }); } catch (e) {}
        const tmp = file + '.tmp';
        // batch 不受 MAX_QUEUE 限制(它们全部让位于实时请求),但仍封顶防滥用
        if (queue.filter(j => j.pr === 'batch').length >= 120) continue;
      ...
      pump();
      sendJson(res, 200, { ok: true, cached, queued });
    } catch (e) {
