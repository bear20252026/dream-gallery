...
function sendJson(res, code, data) {
  ...
  // 2026-09-18 审计 P1#4:不再无条件覆盖为 *；若 handler 已设 ACAO 则沿用
  // 单测 mock res 可能无 getHeader,按未设置处理
  const hasAco = typeof res.getHeader === 'function' && res.getHeader('Access-Control-Allow-Origin');
  if (!hasAco) {
