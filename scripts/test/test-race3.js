...
(async () => {
  ...
  const reqs = [];
  page.on('response', q => { if (q.url().includes('race-check')) reqs.push(q.status() + ' ' + q.url()); });
  ...
  await page.evaluate(() => { window.__ctx.texAllowed=null; window.__tex = window.__ctx.loadTexCapped('photos/race-check.jpg'); });
  await page.waitForTimeout(1500);
  await page.evaluate((m) => { window.__ctx.myUploadTokens = window.__ctx.myUploadTokens || {}; window.__ctx.myUploadTokens['race-check.jpg'] = m; }, '2683816d4f8ec968baa2');
