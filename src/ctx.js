...
// ctx 现为全站跨模块通道(148 个映射属性已收入 7 个命名空间,见文件底部别名层;2026-09-18 收编 13 个散落扁写后核数)。为避免"上帝总线"继续膨胀,规矩:
//   ① 新挂属性必须先在本文件的对应分组登记(写明类型/写入方/用途);
//   ② 能收进深模块的不要挂总线——存档走 ctx.store,弹层走 ctx.overlay,媒体规则走 mediarules.mjs;
//   ③ 分型(命名空间化)**阶段一~五已全部上线**(2026-07-28 一~三 / 2026-08-29 四~五):
//      148 属性已收进 7 命名空间、扁平写软冻结、扁平写零残留 —— 详见 RFC-架构深化.md 候选①。
/*
