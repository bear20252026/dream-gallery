...
// 技术路线(2026-09-02 调研定稿): 描摹矢量 → 分层 Shape 挤出/平铺 → AABB 碰撞注册
// 结构: 外轮廓墙环(带西门洞) + 分层天花(深缝/斜面/亮面) + 7 处白色自发光灯 + 地板
...
const GZ = 8;
const WALL_H = DATA.meta.wallHeight; // 10m
...
const baseCeil = flatMesh(
  ...
  CEIL_BASE,
  new THREE.MeshStandardMaterial({ color: 0x6f6b68, roughness: 0.95, side: THREE.DoubleSide })
);
g.add(baseCeil);
...
lightsSorted.forEach((L, i) => {
  if (i < 4) {
    const pl = new THREE.PointLight('#fff5e8', 8, 32, 1.5);
