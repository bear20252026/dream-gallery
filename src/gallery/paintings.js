...
const {s,cam,ray,mP2,iG,tL,loadTexCapped,vidMesh,v45Mesh,signMesh,wb,mpMesh,vidEl,v45El,signMat,mpMat,OL,OR,OT,OBR}=ctx;

// ===================== 挂画系统（版本27验证方式）=====================
const vE=[], // tL 由 scene.js 创建并经 ctx 共享
fW=new THREE.MeshStandardMaterial({color:'#5a3020',roughness:0.7}),mM=new THREE.MeshStandardMaterial({color:'#fff8f5',roughness:0.95});
function iV(u){return u.endsWith('.mp4')||u.endsWith('.webm');}
// 媒体与配文一一绑定(下标不再依赖取模,动态追加的新媒体也不会错位)
let mi=0;
const aM=P.map((u,i)=>({u:u,d:AI_DESC[i]})).concat(V.map((u,j)=>({u:u,d:AI_DESC[P.length+j]})));
...
(function(){
  const SLOT_N=6,SLOT_W=1.6,SLOT_H=1.2,GAP=0.5;
  const WALL_W=SLOT_N*SLOT_W+(SLOT_N+1)*GAP,WALL_H=2.6,WALL_Z=47;
  // 展示墙:浅绿色半透明玻璃
  const wallMesh=new THREE.Mesh(
    new THREE.BoxGeometry(WALL_W,WALL_H,0.15),
    new THREE.MeshPhysicalMaterial({color:'#8fe0b8',transparent:true,opacity:0.32,roughness:0.08,metalness:0.1,side:THREE.DoubleSide})
  );
  wallMesh.position.set(0,WALL_H/2,WALL_Z);s.add(wallMesh);
  ...
  function makeTitleTex(){
    ...
    tX.fillStyle='rgba(90,190,255,0.35)';tX.fillRect(0,0,512,80);
    tX.strokeStyle='rgba(140,220,255,0.9)';tX.lineWidth=3;tX.strokeRect(4,4,504,72);
    tX.fillStyle='#bfeaff';tX.font='bold 44px sans-serif';tX.textAlign='center';
    tX.shadowColor='#4fc3ff';tX.shadowBlur=14;
    tX.fillText('白板作品展',256,55);
    const tT=new THREE.CanvasTexture(tC);tT.colorSpace=THREE.SRGBColorSpace;
    return tT;
  }
  const tT=makeTitleTex();
  const titleGlassMat=function(){return new THREE.MeshBasicMaterial({map:tT,transparent:true,opacity:0.75,side:THREE.FrontSide,toneMapped:false});};
  // 北面(朝向建筑 -z)正字
  const titleN=new THREE.Mesh(new THREE.PlaneGeometry(4,0.625),titleGlassMat());
  titleN.rotation.y=Math.PI;titleN.position.set(0,WALL_H+0.55,WALL_Z);s.add(titleN);
  // 南面(朝向白板 +z)正字
  const titleS=new THREE.Mesh(new THREE.PlaneGeometry(4,0.625),titleGlassMat());
  titleS.position.set(0,WALL_H+0.55,WALL_Z);s.add(titleS);
  // 6 个固定画位（占位画布，加载到作品后替换）
  const slots=[];
  // 共享占位纹理
  const ph=document.createElement('canvas');ph.width=256;ph.height=192;
  const px=ph.getContext('2d');
  px.fillStyle='#fff';px.fillRect(0,0,256,192);
  px.fillStyle='#bbb';px.font='20px sans-serif';px.textAlign='center';
  px.fillText('虚位以待',128,100);
  const pTex=new THREE.CanvasTexture(ph);pTex.colorSpace=THREE.SRGBColorSpace;
  for(let i=0;i<SLOT_N;i++){
    const x=-WALL_W/2+GAP+SLOT_W/2+i*(SLOT_W+GAP);
    const g=new THREE.Group();
    g.position.set(x,1.5,WALL_Z-0.15);
    g.rotation.y=Math.PI; // 面向白板方向(-z)
    g.userData={isPainting:true,ox:x,oy:1.5,oz:WALL_Z-0.15,nx:0,nz:-1,ry:Math.PI,zoomed:false,aiDesc:'白板手绘作品'};
    s.add(g);iG.push(g);
  ...
  function refreshWall(){
    fetch('/api/files?dir=photos').then(function(r){return r.json()}).then(function(d){
      const files=(d.photos||[]).filter(function(f){return /^whiteboard-.*\.png$/i.test(f.name)})
        .sort(function(a,b){return b.mtime.localeCompare(a.mtime)}).slice(0,SLOT_N);
      for(let i=0;i<SLOT_N;i++){
        const f=files[i],slot=slots[i];
        const newName=f?f.name:'';
        if(newName===slot.name)continue; // 画位内容没变，跳过
        slot.name=newName;
        if(!f){ // 作品被删除，恢复占位
          slot.cm.map=pTex;slot.cm.needsUpdate=true;
        ...
        tL.load(f.url,function(tex){
          ...
          slot.cm.map=tex;slot.cm.needsUpdate=true;
          if(oldTex&&oldTex!==pTex)oldTex.dispose(); // 释放旧纹理,占位纹理除外
          slot.g.userData.aiDesc='白板手绘作品 · '+f.name.replace(/^whiteboard-|\.\w+$/g,'').replace('T',' ').slice(0,19);
        });
      }
    }).catch(function(e){console.warn('白板作品墙刷新失败:',e);});
  }
  refreshWall();
  ...
  document.addEventListener('visibilitychange',function(){
    if(document.hidden){clearInterval(wallTimer)}
    else{refreshWall();wallTimer=setInterval(refreshWall,60000)}
  });
})();
...
function hangOn(wall,wi,am,off){
  off=off||0;
  const mU=am.u,g=new THREE.Group();
  const pos=wall.position.clone(),rot=wall.rotation.y;
  const pX=Math.cos(rot),pZ=-Math.sin(rot);
  // 凸出0.25m：贴墙但不悬空，不被遮挡;off 为沿墙方向的偏移(避让已有画框)
  const dirX=Math.sin(rot),dirZ=Math.cos(rot);
  pos.x+=dirX*off+pX*0.25;pos.z+=dirZ*off+pZ*0.25;pos.y=2.2;
  g.position.copy(pos);g.rotation.y=Math.atan2(pX,pZ);
  // 3D放大交互数据(配文随媒体绑定);src 供模式系统识别来源(图库/演示/本人上传)
  g.userData={isPainting:true,ox:pos.x,oy:pos.y,oz:pos.z,nx:pX,nz:pZ,ry:g.rotation.y,zoomed:false,aiDesc:am.d,src:mU};
  iG.push(g); // 直接收集可交互画框（版本57方式，可靠）
  paintGroups.push(g);
  const fw=1,fh=1.4,fd=0.07;
  ...
  hitBox.position.z=0.1;g.add(hitBox);
  const mt=new THREE.Mesh(new THREE.BoxGeometry(fw-0.12,fh-0.12,0.012),mM);
  mt.position.z=fd/2+0.004;g.add(mt);
  const cg=new THREE.PlaneGeometry(fw-0.22,fh-0.22);
  let cm;
  if(iV(mU)){
    const v=document.createElement('video');v.src=mU;v.crossOrigin='anonymous';v.loop=true;v.muted=true;v.playsInline=true;v.preload='none';v.style.display='none';document.body.appendChild(v);vE.push({v,x:pos.x,z:pos.z});
    const vt=new THREE.VideoTexture(v);vt.colorSpace=THREE.SRGBColorSpace;
    cm=new THREE.MeshStandardMaterial({map:vt,roughness:0.3,emissive:'#ffffff',emissiveIntensity:0.08});
    // 不在这里 play:十几路视频同时起播会拖垮弱网与解码(2026-07-24 血泪教训),由下方就近播放管理器统一调度
  }else{
    // 修复2：图片加载+错误处理，失败时显示粉色占位;pos 供距离懒加载(走近才拉纹理)
    const tex=loadTexCapped(mU,
      ()=>{
        const pc=document.createElement('canvas');pc.width=256;pc.height=256;
        const px=pc.getContext('2d');
        px.fillStyle='#ffb6c8';px.fillRect(0,0,256,256);
        px.fillStyle='#fff';px.font='bold 20px Arial';px.textAlign='center';
        px.fillText('Photo',128,120);px.fillText('Loading...',128,150);
        const pt=new THREE.CanvasTexture(pc);pt.colorSpace=THREE.SRGBColorSpace;
        cm.map=pt;cm.needsUpdate=true;
      },
      {x:pos.x,z:pos.z} // 距离懒加载坐标
    );
    cm=new THREE.MeshStandardMaterial({map:tex,roughness:0.4});
  }
  const cM=new THREE.Mesh(cg,cm);cM.position.z=fd/2+0.012;g.add(cM);s.add(g);
  ...
  ghost.position.z=fd/2+0.02;g.add(ghost);
  if(wi<40){const sp=new THREE.SpotLight('#ffe8f0',3,5,Math.PI/4,0.8,1);sp.position.set(0,0.7,0.35);sp.target=cM;g.add(sp);g.add(sp.target);}
  return g;
}
...
function rippleAt(g){
  const ring=new THREE.Mesh(new THREE.RingGeometry(0.6,0.66,48),new THREE.MeshBasicMaterial({color:'#ffd88a',transparent:true,opacity:0.85,side:THREE.DoubleSide,depthWrite:false}));
  ring.position.set(g.userData.ox+g.userData.nx*0.4,g.userData.oy,g.userData.oz+g.userData.nz*0.4);
  ring.rotation.y=Math.atan2(g.userData.nx,g.userData.nz);
  s.add(ring);
  const t0=performance.now();
  (function ripple(){
    const p=Math.min((performance.now()-t0)/1200,1);
    ring.scale.setScalar(1+p*3.2);ring.material.opacity=0.85*(1-p);
    if(p<1)requestAnimationFrame(ripple);else{s.remove(ring);ring.geometry.dispose();ring.material.dispose();}
  })();
}
// 空框换芯(2026-07-27 主人定:框留下,照片拿掉,新上传的照片使用这些框)
// 普通模式下,新照片优先替换一个图库空框的内容面;画框位置/外框不动,来源改记本人上传(模式系统因此放行)
function frameDirOf(g){const a=Math.atan2(g.userData.nz,g.userData.nx);return Math.abs(a)<=Math.PI/4?'E':Math.abs(a)>=3*Math.PI/4?'W':a>0?'S':'N';}
function takeoverEmpty(mU,caption,aura){
  const candidates=(ctx.gallery.paintGroups||[]).filter(g=>{
    const src=g.userData.src||'',n=src.split('/').pop();
    return LIB_SET.has(src)&&g.userData.empty&&!/\.(mp4|webm|mov|m4v|mkv|avi)$/i.test(src)
      &&!(ctx.mode.myUploads||[]).includes(n)&&!(ctx.mode.demoPhotos||[]).includes(n);
  });
  ...
  cm.material.map=tex;cm.material.needsUpdate=true;cm.visible=true;
...
function wallDir(wall){
  const r=wall.rotation.y,nx=Math.cos(r),nz=-Math.sin(r),a=Math.atan2(nz,nx);
  return Math.abs(a)<=Math.PI/4?'E':Math.abs(a)>=3*Math.PI/4?'W':a>0?'S':'N';
}
...
function wallHalf(wall){
  if(wall.userData._half===undefined){
    ...
    const r=wall.rotation.y,dx=Math.abs(Math.sin(r)),dz=Math.abs(Math.cos(r));
...
function hangOne(mU,caption,aura){
  ...
  if(ctx.mode.siteMode==='normal'&&/\.(jpe?g|png|webp|gif|bmp|avif|heic|heif|tiff?)$/i.test(mU)){
  ...
  function findSpot(walls){
    for(const wall of walls){
      ...
      for(const off of offs){
        ...
        const rot=wall.rotation.y,dirX=Math.sin(rot),dirZ=Math.cos(rot);
        const px=wall.position.x+dirX*off,pz=wall.position.z+dirZ*off;
        let clash=false;
        for(const g of paintGroups){
          if(Math.hypot(g.userData.ox-px,g.userData.oz-pz)<1.0){clash=true;break;}
        }
  ...
  // 昆仑灵鉴 M4:归位涟漪——金色圆环在画框上荡开,宣告"它归位了"
  {
    const ring=new THREE.Mesh(new THREE.RingGeometry(0.6,0.66,48),new THREE.MeshBasicMaterial({color:'#ffd88a',transparent:true,opacity:0.85,side:THREE.DoubleSide,depthWrite:false}));
    ring.position.set(g.userData.ox+g.userData.nx*0.4,g.userData.oy,g.userData.oz+g.userData.nz*0.4);
    ring.rotation.y=Math.atan2(g.userData.nx,g.userData.nz);
...
fetch('/api/files').then(function(r){return r.json()}).then(function(d){
  ...
  const newP=(d.photos||[]).map(function(f){return f.url.slice(1)}).filter(function(u){return !known.has(u)&&!/^photos\/whiteboard-/i.test(u)});
  const newV=(d.videos||[]).map(function(f){return f.url.slice(1)}).filter(function(u){return !known.has(u)});
  for(let i=newP.length-1;i>=0;i--)aM.unshift({u:newP[i],d:'新上传的照片'});
  for(let i=newV.length-1;i>=0;i--)aM.unshift({u:newV[i],d:'新上传的视频'});
  hangPaintings();
}).catch(function(){hangPaintings()});
...
function onC3D(e){
  ...
  if(hs.length===0){if(ctx.gallery.zG)zoomOut(ctx.gallery.zG);return;} // 点空白=关闭放大(退出普查 2026-07-26)
  if(hs.length>0){
    let cg=hs[0].object;
    ...
    if(!ctx.player.quizPassed&&!(cg.userData&&cg.userData.isQuizGate)&&cg.position.x>-19&&cg.position.x<19&&cg.position.z>-13&&cg.position.z<29)return;
    // ===== 户外白板点击:内嵌面板打开,不脱离画廊 =====
    if(cg.userData&&cg.userData.isWB){
      window.openPanel('whiteboard.html','希沃白板');return;
    }
    ...
    if(cg.userData&&cg.userData.isMusic){
      ...
      mpMat.emissiveIntensity=0.5;setTimeout(function(){mpMat.emissiveIntensity=0.15;},300);
      return;
    }
    // ===== AI牌子点击:内嵌面板打开 =====
    if(cg.userData&&cg.userData.isSign){
      window.openPanel('https://page.goose.cc.cd/s/Hi-AI-2-0','外部链接');
      signMat.emissiveIntensity=0.5;setTimeout(function(){signMat.emissiveIntensity=0.15;},300);
      return;
    }
    // ===== 元素共鸣准则牌子:打开用户说明书 =====
    ...
    if(cg.userData&&cg.userData.isQuizGate){
      window.startQuiz&&window.startQuiz();return;
    }
    // ===== 永恒展厅(eternal.js)通用交互钩子:金门/返程金拱门等 =====
    if(cg.userData&&cg.userData.eternalAction){if(ctx.kunlun.eternalClick)ctx.kunlun.eternalClick(cg);return;}
    if(cg.userData&&cg.userData.isPainting&&!cg.userData.empty){ // 空框(图库隐藏帧)不可点,不放大
      if(ctx.gallery.zG&&ctx.gallery.zG!==cg)zoomOut(ctx.gallery.zG,true); // 切换放大对象:旧的直接复位,避免动画竞态卡死
      if(!cg.userData.zoomed){zoomIn(cg);}
      else{zoomOut(cg);}
      return;
    }
    // 然后检测3D墙面问卷面板
    ...
    if(cg.userData){
      ...
      if(linkKey){
        ...
        window.openPanel(LINKS[linkKey],'外部链接');return;
      }
    }
  }
...
const aiPanel=document.getElementById('aiPanel'),aiT=document.getElementById('aiT');
function showAI(text){aiT.textContent=/^昆仑替你记得：/.test(text)?text:'昆仑替你记得：'+text;aiPanel.classList.add('show');}
function hideAI(){aiPanel.classList.remove('show');}
...
function captionAllowed(g){
  ...
  return MR.captionAllowed({mode:ctx.mode.siteMode,
...
function zoomIn(cg){
  const d=cg.userData;
  d.zoomed=true;ctx.gallery.zG=cg;
  d.gazeT0=Date.now(); // 昆仑灵鉴 M2:凝视计时起点
  if(!oFog)oFog=s.fog.density;
  // 添加环境光晕
  const hl=new THREE.PointLight('#ffc8e0',5,8,1.5);
  hl.position.set(0,0.3,0.8);cg.add(hl);cg.userData.hl=hl;
  // 添加背光（凸显轮廓）
  const bl=new THREE.PointLight('#80c8ff',2,6,1.5);
  bl.position.set(0,0,-0.5);cg.add(bl);cg.userData.bl=bl;
  // 动画参数
  const dur=500; // 500ms飞入
  const t0=performance.now();
  const sx=cg.position.x,sy=cg.position.y,sz=cg.position.z;
  const ss=cg.scale.x;
  // 目标：沿法线推进0.5m，正对相机，缩放2x
  const tx=d.ox+d.nx*0.5,tz=d.oz+d.nz*0.5;
  ...
  function animateIn(){
    ...
    if(p<1){d.aF=requestAnimationFrame(animateIn);}
    else{
      // 到达后物理摇晃（弹性回弹，像挂在弹簧上）
...
function zoomOut(cg,instant){
  ...
  if(d.aF)cancelAnimationFrame(d.aF);
  if(d.hl){cg.remove(d.hl);d.hl=null;}
  if(d.bl){cg.remove(d.bl);d.bl=null;}
  if(instant){
    cg.position.set(d.ox,d.oy,d.oz);cg.rotation.y=d.ry;cg.scale.set(1,1,1);
    d.zoomed=false;
  ...
  function animateOut(){
    ...
    if(p<1){d.aF=requestAnimationFrame(animateOut);}
    else{
      d.zoomed=false;
      if(ctx.gallery.zG===cg){ctx.gallery.zG=null;hideAI();} // 竞态防护:期间已有新画放大则不抢状态
      s.fog.density=oFog;
    }
  }
  animateOut();
...
setInterval(function(){
  ...
  const px=ctx.player.pl.p.x,pz=ctx.player.pl.p.z;
  for(const e of vE){
    // 普通模式:图库视频不播(2026-07-25 主人修订:本人上传的两模式都可播;决策表=shared/mediarules.cjs)
    const vn=(e.v.src||'').split('/').pop();
    const vidAllowed=MR.contentAllowed({mode:ctx.mode.siteMode,
      isDemo:(ctx.mode.demoPhotos||[]).includes(vn),
      isMine:(ctx.mode.myUploads||[]).includes(vn),
      isLib:true}); // 非本人/非演示的挂画视频按图库论:普通模式不播
    if(!vidAllowed){if(!e.v.paused)e.v.pause();continue;}
    const d2=(e.x-px)*(e.x-px)+(e.z-pz)*(e.z-pz);
    if(d2<18*18){if(e.v.paused)e.v.play().catch(function(){});}
    else if(d2>22*22&&!e.v.paused)e.v.pause();
  }
},500);

Object.assign(ctx.gallery,{onC3D,zoomOut});ctx.zoomIn=zoomIn; // 命名空间注册(zoomIn 仅本模块外用潜力,暂留扁平)
