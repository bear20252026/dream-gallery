...
function speak(t) {
  try {
    ctx.ui.kunlunSpeak && ctx.ui.kunlunSpeak(t);
  } catch (e) {}
}
...
function mirrorSVG() {
  ...
  let cracks = '';
  ends.forEach((e, i) => {
    const mx = (cx + e[0]) / 2 + (i % 2 ? 14 : -12),
      my = (cy + e[1]) / 2 + (i % 3 ? -10 : 12);
    cracks +=
      '<path d="M' +
      cx +
      ' ' +
      ...
      ' ' +
      e[1] +
      '" stroke="rgba(220,200,170,.55)" stroke-width="1" fill="none"/>';
  });
  return (
    '<svg viewBox="0 0 200 280" style="width:min(46vw,240px);height:auto;display:block;filter:drop-shadow(0 0 40px rgba(160,120,60,.25))">' +
...
function build() {
  ...
  skip.style.cssText =
    'position:absolute;right:16px;bottom:14px;z-index:502;background:none;border:none;color:rgba(255,255,255,.28);font-size:11px;letter-spacing:2px;cursor:pointer;font-family:inherit';
...
function line(txt, ms, tts) {
  return new Promise((res) => {
    ...
    d.style.cssText =
      'position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);max-width:84vw;text-align:center;font-size:clamp(20px,5vw,30px);letter-spacing:4px;color:#e8dcc4;text-shadow:0 2px 18px rgba(0,0,0,.9);line-height:2;opacity:0;transition:opacity 1s';
    d.textContent = txt;
    ov.appendChild(d);
...
// 重构:去掉 new Promise(async executor) 反模式——async 函数本身即返回 Promise
async function replay() {
  for (let i = 0; i < SCENES.length; i++) {
    if (dead) return;
    const d = document.createElement('div');
    d.style.cssText =
      'position:absolute;left:50%;top:62%;transform:translateX(-50%);width:86vw;text-align:center;font-size:clamp(14px,3.4vw,18px);letter-spacing:2px;color:' +
      SCENES[i][1] +
      ';text-shadow:0 1px 10px rgba(0,0,0,.9);line-height:1.9;opacity:0;transition:opacity .9s';
    d.textContent = SCENES[i][0];
    ov.appendChild(d);
    requestAnimationFrame(() => {
      d.style.opacity = '1';
    });
...
function choose() {
  return new Promise((res) => {
    ...
    q.style.cssText =
      'position:absolute;left:50%;top:66%;transform:translateX(-50%);text-align:center;opacity:0;transition:opacity 1.2s';
    q.innerHTML =
      '<div style="font-size:clamp(17px,4.4vw,24px);letter-spacing:3px;color:#ffe9c4;text-shadow:0 0 24px rgba(255,200,100,.5);margin-bottom:18px">你愿意让它重新亮起来吗？</div>';
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;gap:14px;justify-content:center';
    const yes = document.createElement('button');
    yes.textContent = '我愿意';
    const no = document.createElement('button');
    no.textContent = '让我再想想';
    const bs =
      'padding:12px 30px;border-radius:26px;font-size:15px;letter-spacing:3px;cursor:pointer;font-family:inherit;';
    yes.style.cssText =
      bs + 'border:1px solid rgba(255,214,130,.85);background:rgba(120,80,25,.45);color:#ffe9c4';
    no.style.cssText =
      bs +
      'border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.06);color:rgba(255,255,255,.75)';
    const bar = document.createElement('div'); // 3 秒自动"我愿意"倒计时细条
    bar.style.cssText =
      'margin:16px auto 0;width:180px;height:2px;background:rgba(255,214,130,.15);position:relative;overflow:hidden;border-radius:2px';
    const fill = document.createElement('div');
    fill.style.cssText =
      'position:absolute;left:0;top:0;bottom:0;width:0;background:rgba(255,214,130,.8);transition:width 3s linear';
    ...
    const fin = (v) => {
      ...
      // ⚠️ 必须在用户手势回调内立即启动音视频序列!
      // 否则后续 setTimeout/await 会断开手势链,Audio.play() 被 Chrome 拦截
...
function glowAndFade() {
  return new Promise((res) => {
    ...
    if (cr) {
      cr.style.transition = 'opacity 1.2s';
      cr.style.opacity = '0';
    }
    const glow = document.createElement('div');
    glow.style.cssText =
      'position:absolute;left:50%;top:44%;transform:translate(-50%,-50%);width:60vmin;height:60vmin;border-radius:50%;background:radial-gradient(circle,rgba(255,214,130,.55),rgba(255,180,80,.18) 55%,transparent 72%);opacity:0;transition:opacity 1.4s;pointer-events:none';
    ov.appendChild(glow);
    requestAnimationFrame(() => {
...
function leaveDimMirror() {
  ...
  mirrorBtn.style.cssText =
    'position:fixed;right:14px;bottom:96px;z-index:60;width:44px;height:44px;border-radius:50%;border:1px solid rgba(200,170,120,.4);background:rgba(20,16,12,.75);font-size:20px;cursor:pointer;opacity:.65;filter:grayscale(.6)';
...
function finish(openQuizAfter) {
  ...
  // startVidSeq 已在 yes.onclick 中调用（用户手势链内），此处不再重复
  const done = () => {
    if (ovApi) {
      ovApi.unregister();
      ovApi = null;
    ...
    if (mirrorBtn && openQuizAfter !== false) {
      mirrorBtn.remove();
      mirrorBtn = null;
    }
    if (openQuizAfter) {
...
async function run() {
  ...
  mir.style.cssText =
    'position:absolute;left:50%;top:44%;transform:translate(-50%,-50%) scale(.92);opacity:0;transition:opacity 1.6s,transform 1.6s';
  mir.innerHTML = mirrorSVG();
  ov.appendChild(mir);
  requestAnimationFrame(() => {
    mir.style.opacity = '1';
    mir.style.transform = 'translate(-50%,-50%) scale(1)';
  });
...
// 新用户: finish() 里调用(用户已点"我愿意"/"跳过"=已交互); 老用户: 首次点击时调用。
function _startOnInteract() {
  if (ctx.stopAgreementMusic) ctx.stopAgreementMusic();
  if (ctx.startVidSeq) ctx.startVidSeq();
}
if (!ctx.store.flag('prologueDone') && !/noprologue/.test(location.search)) {
  ...
  // 老用户跳过序章:首次交互时启动画廊音乐+视频序列(遵守浏览器自动播放策略)
  document.addEventListener('click', _startOnInteract, { once: true });
  document.addEventListener('touchstart', _startOnInteract, { once: true });
  // C6 回归文案(2026-07-28,设计文档防流失):老访客再来,残镜小字静候(每会话一次,有进度才说)
  if (
    !sessionStorage.getItem('kunlunWelcomeBack') &&
    (ctx.store.num('quiz') > 0 || ctx.store.num('up') > 0)
  ) {
    sessionStorage.setItem('kunlunWelcomeBack', '1');
    setTimeout(() => {
      ctx.ui.modeToast && ctx.ui.modeToast('上次你带来了几片灵蕴。它们还在等你。');
    }, 6000);
  }
}
