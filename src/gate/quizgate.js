...
toastEl.style.cssText='position:fixed;top:70px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,.8);color:#feca57;padding:10px 24px;border-radius:24px;font-size:14px;z-index:500;display:none;pointer-events:none;font-family:"Microsoft YaHei",sans-serif;letter-spacing:1px;';
document.body.appendChild(toastEl);
let toastTimer=0,lastToast=0;
...
// ===================== 通行证状态 =====================
if(ctx.player.quizPassed===undefined)ctx.player.quizPassed=false; // 热更新重载时保留已通过状态
if(ctx.player.quizPassScore===undefined)ctx.player.quizPassScore=60; // 分数线(服务端单源下发前的兜底,与 lib/quiz.js 一致)
fetch('/api/quiz/state').then(r=>r.json()).then(d=>{
  if(d.passScore)ctx.player.quizPassScore=d.passScore; // 分数线单一源:lib/quiz.js QUIZ_PASS_SCORE 下发
  if(d.passed)unlockGallery(false);
}).catch(()=>{});

// ===================== 实体门禁墙(封顶盒子,彻底挡住建筑内外视线,≥60分自动沉入地面消失;未达60可领特别邀请函进馆) =====================
const gateWalls=new THREE.Group();
let gateSign=null; // 围墙告示牌(提升作用域:热更新清理 iG 时要引用)
(function buildGateWalls(){
  const wallMat=new THREE.MeshStandardMaterial({color:'#4a2c1a',roughness:0.75,metalness:0.1,side:THREE.DoubleSide});
  const trimMat=new THREE.MeshStandardMaterial({color:'#c9a050',roughness:0.35,metalness:0.7});
  // 四面墙高 8 米,段长比建筑外圈多 1 米,确保拐角无缝隙
  function seg(cx,cz,w,d){
    const m=new THREE.Mesh(new THREE.BoxGeometry(w,10,d),wallMat);
    m.position.set(cx,5,cz);gateWalls.add(m);
    const t=new THREE.Mesh(new THREE.BoxGeometry(w===0.4?0.5:w+0.1,0.25,d===0.4?0.5:d+0.1),trimMat);
    t.position.set(cx,10.05,cz);gateWalls.add(t);
  }
  seg(0,28.3,38,0.4);   // 南墙(面向出生点)
  seg(0,-12.3,38,0.4);  // 北墙
  seg(-18.3,8,0.4,42);  // 西墙
  seg(18.3,8,0.4,42);   // 东墙
  // 顶部盖板:彻底封死俯瞰视线
  const cap=new THREE.Mesh(new THREE.BoxGeometry(38,0.3,42),wallMat);
  cap.position.set(0,10.1,8);gateWalls.add(cap);
  // 南面告示牌
  const cv=document.createElement('canvas');cv.width=512;cv.height=128;
  const c=cv.getContext('2d');
  c.fillStyle='#4a2c1a';c.fillRect(0,0,512,128);
  c.strokeStyle='#c9a050';c.lineWidth=6;c.strokeRect(6,6,500,116);
  c.fillStyle='#feca57';c.font='bold 44px "Microsoft YaHei",serif';c.textAlign='center';
  c.fillText('心 象 共 鸣',256,58);
  c.fillStyle='rgba(255,255,255,0.85)';c.font='26px "Microsoft YaHei",serif';
  c.fillText('点 击 开 始',256,100);
  const tex=new THREE.CanvasTexture(cv);tex.colorSpace=THREE.SRGBColorSpace;
  gateSign=new THREE.Mesh(new THREE.PlaneGeometry(8,2),new THREE.MeshBasicMaterial({map:tex}));
  gateSign.position.set(0,4.2,28.55);gateWalls.add(gateSign); // 面向+z(出生点方向)
  // 围墙告示牌可点击:点击走查会向上找到 gateWalls(场景直接子级),标记打在组上
  gateWalls.userData.isQuizGate=true;
  iG.push(gateSign);
})();
s.add(gateWalls);
// 门禁墙碰撞体:加入玩家碰撞系统,墙体消失前相机无法穿墙(视野穿透的根因)
const gateBounds=[
  {mnX:-19,mxX:19,mnZ:28.1,mxZ:28.5},   // 南墙
  {mnX:-19,mxX:19,mnZ:-12.5,mxZ:-12.1}, // 北墙
  {mnX:-18.5,mxX:-18.1,mnZ:-13,mxZ:29}, // 西墙
  {mnX:18.1,mxX:18.5,mnZ:-13,mxZ:29},   // 东墙
];
if(ctx.scene.bounds)gateBounds.forEach(b=>ctx.scene.bounds.push(b));

function unlockGallery(notify){
  ctx.player.quizPassed=true;
  drawPanel(true);
  gateWalls.visible=true;
  // 移除门禁墙碰撞体
  if(ctx.scene.bounds)gateBounds.forEach(b=>{const i=ctx.scene.bounds.indexOf(b);if(i>=0)ctx.scene.bounds.splice(i,1);});
  // 墙体沉入地面动画(2秒),然后隐藏(保留以便重新竖起)
  const t0=performance.now();
  (function sink(){
    const p=Math.min((performance.now()-t0)/2000,1);
    const ease=p*p;
    gateWalls.position.y=-11*ease;
    if(p<1)requestAnimationFrame(sink);
    else{gateWalls.visible=false;gateWalls.position.y=0;}
  })();
  if(notify)window.quizToast('🎉 解密成功!门禁墙已消失,欢迎进入画廊');
}

// 重新竖起门禁墙(权限被撤销时):送出建筑内的人,墙回到原位,碰撞恢复
function lockGallery(reason){
  ctx.player.quizPassed=false;
  drawPanel(false);
  gateWalls.visible=true;
  if(ctx.scene.bounds)gateBounds.forEach(b=>{if(!ctx.scene.bounds.includes(b))ctx.scene.bounds.push(b);});
  // 若玩家还在建筑内,传送回出生点(送出建筑,不踢出网站)
  if(ctx.player.pl&&inBuildingRect(ctx.player.pl.p.x,ctx.player.pl.p.z)){
    ctx.player.pl.p.set(20,1.6,14);
    if(ctx.scene.cam){ctx.scene.cam.position.copy(ctx.player.pl.p);}
  }
  window.quizToast(reason||'建筑访问权限已撤销,门禁墙已重新竖起');
}
function inBuildingRect(x,z){return x>-19&&x<19&&z>-13&&z<29;}

// 权限监视:每5秒检查一次——被授予时墙沉下,被撤销时墙竖起并送出
setInterval(async function(){
  try{
    const r=await fetch('/api/quiz/state');
    const d=await r.json();
    if(d.passed&&!ctx.player.quizPassed)unlockGallery(true);        // 后台授予VIP,实时解锁
    else if(!d.passed&&ctx.player.quizPassed)lockGallery('访问权限已被撤销,你已被送出建筑');
  }catch(e){}
},5000);

// ===================== 悬浮答题屏(3D) =====================
const panelCv=document.createElement('canvas');panelCv.width=1024;panelCv.height=512;
...
function drawPanel(unlocked){
  ...
  c.beginPath();c.roundRect(24,24,976,464,36);c.stroke();
  c.strokeStyle='rgba(255,120,170,0.6)';c.lineWidth=3;
  c.beginPath();c.roundRect(36,36,952,440,28);c.stroke();
  c.textAlign='center';
  if(unlocked){
    c.fillStyle='#2e9e6b';c.font='bold 130px "Microsoft YaHei",serif';
    c.shadowColor='rgba(255,255,255,0.95)';c.shadowBlur=18;
    c.fillText('已 解 锁',512,240);
    c.shadowBlur=0;
    c.fillStyle='rgba(60,40,60,0.8)';c.font='46px "Microsoft YaHei",serif';
    c.fillText('欢迎光临梦幻画廊',512,340);
  }else{
    c.fillStyle='#ffffff';c.font='bold 108px "Microsoft YaHei",serif';
    c.shadowColor='rgba(255,255,255,0.9)';c.shadowBlur=14;
    c.fillText('心 象 共 鸣',512,205);
    c.shadowBlur=0;
    c.fillStyle='rgba(255,255,255,0.95)';c.font='bold 54px "Microsoft YaHei",serif';
    c.fillText('9 道选择 + 1 道问答 · 满分 100',512,300);
    c.fillStyle='rgba(255,255,255,0.9)';c.font='44px "Microsoft YaHei",serif';
...
const panelMesh=new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({map:panelTex,side:THREE.DoubleSide,transparent:true,opacity:0.62,toneMapped:false})
);
panelMesh.position.set(39,8,14);
panelMesh.rotation.y=-Math.PI/2; // 面向-x(出生点 x=20 方向)
panelMesh.userData={isQuizGate:true};
s.add(panelMesh);iG.push(panelMesh);
// 玫瑰金边框(半透明) + 氛围灯 + 浮动动画
const panelFrame=new THREE.Mesh(
  new THREE.BoxGeometry(14.4,7.4,0.15),
  new THREE.MeshStandardMaterial({color:'#d98aa5',roughness:0.35,metalness:0.55,transparent:true,opacity:0.5})
);
panelFrame.position.set(39.12,8,14);panelFrame.rotation.y=-Math.PI/2;s.add(panelFrame);
const panelLight=new THREE.PointLight('#ffb6c8',3,20,1.5);
panelLight.position.set(36,8,14);s.add(panelLight);
onTick(function(){
...
const QZ_TITLES=['第一问·观天地','第二问·察万物','第三问·辨时序','第四问·明因果','第五问·识变迁','第六问·通情理','第七问·分虚实','第八问·知进退','第九问·度深浅'];
...
const ov=document.createElement('div');ov.id='quizOv';
const box=document.createElement('div');box.id='quizBox';
ov.appendChild(box);
document.body.appendChild(ov);

// ===================== 昆仑灵鉴 V1:答题动态背景(灰暗天空+金色裂纹,作答越深入裂纹越亮,满分金光填满现昆仑) =====================
const sky=document.createElement('canvas');
sky.style.cssText='position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:0';
ov.insertBefore(sky,box);
box.style.position='relative';box.style.zIndex='1';
const skyX=sky.getContext('2d');
let skyRatio=0,skyPts=[],skyBranches=[],skyAnim=0;
function skySeedPts(){
  skyPts=[];skyBranches=[];
  const w=sky.width,h=sky.height,steps=36;let x=w/2;
  for(let i=0;i<=steps;i++){
    const y=-10+(h+20)*i/steps;
    x+=(Math.random()-0.5)*w*0.09;x=Math.max(w*0.18,Math.min(w*0.82,x));
    skyPts.push([x,y]);
    // 雷霆分叉:主干的 16% 节点长出不规则支脉
    if(i>6&&i<steps-4&&Math.random()<0.16){
      let bx=x,by=y;const bp=[[bx,by]],dir=Math.random()<0.5?-1:1;
      for(let j=0;j<6;j++){bx+=dir*(w*0.02+Math.random()*w*0.05);by+=(h/steps)*(0.4+Math.random()*0.8);bp.push([bx,by]);}
      skyBranches.push(bp);
    }
  }
}
...
function bolt(c,pts,w0,r){
  c.strokeStyle=`rgba(255,190,90,${0.10+r*0.25})`;c.lineWidth=w0*5;c.shadowColor='rgba(255,200,100,0.8)';c.shadowBlur=18+r*40;
  c.beginPath();c.moveTo(pts[0][0],pts[0][1]);for(const [x,y] of pts)c.lineTo(x,y);c.stroke();
  c.strokeStyle=`rgba(255,214,130,${0.35+r*0.6})`;c.lineWidth=w0*1.6;c.shadowBlur=8+r*18;c.stroke();
  if(r>0.4){c.strokeStyle=`rgba(255,246,220,${(r-0.4)*1.1})`;c.lineWidth=w0*0.6;c.shadowBlur=4;c.stroke();}
}
function drawSky(){
  const w=sky.width=sky.clientWidth||innerWidth,h=sky.height=sky.clientHeight||innerHeight;
  if(!skyPts.length)skySeedPts();
  const r=skyRatio,c=skyX;
  const g=c.createLinearGradient(0,0,0,h);g.addColorStop(0,'#1b1820');g.addColorStop(1,'#0c0a10');
  c.fillStyle=g;c.fillRect(0,0,w,h);
  // 雷光映云(大范围暖晕,进度越高越亮)
  if(r>0.02){const rg=c.createRadialGradient(w/2,h*0.4,10,w/2,h*0.4,h*0.75);rg.addColorStop(0,`rgba(255,200,110,${r*0.10})`);rg.addColorStop(1,'rgba(255,200,110,0)');c.fillStyle=rg;c.fillRect(0,0,w,h);}
  c.save();c.lineCap='round';c.lineJoin='round';
  bolt(c,skyPts,3+r*4,r);                       // 主雷:随进度变粗变亮
  for(const bp of skyBranches)bolt(c,bp,1.5+r*2,r*0.8); // 支脉
  c.restore();
  if(r>=0.98){ // 金光填满→昆仑山巅剪影
    const rg=c.createRadialGradient(w/2,h*0.35,10,w/2,h*0.35,h*0.9);
    rg.addColorStop(0,'rgba(255,215,140,0.85)');rg.addColorStop(1,'rgba(255,190,90,0)');
    c.fillStyle=rg;c.fillRect(0,0,w,h);
    c.fillStyle='#1c1420';c.beginPath();c.moveTo(0,h);
    const peaks=[[0,.78],[.12,.62],[.22,.74],[.35,.5],[.5,.7],[.62,.55],[.75,.72],[.88,.6],[1,.76]];
...
function skyTo(target,animate){
  ...
  if(!animate){skyRatio=target;drawSky();return;}
  const from=skyRatio,t0=performance.now();
  (function step(){const p=Math.min((performance.now()-t0)/1200,1);skyRatio=from+(target-from)*p;drawSky();if(p<1)skyAnim=requestAnimationFrame(step);})();
}
...
quizCloseBtn.style.cssText='position:absolute;top:14px;right:14px;z-index:401;padding:8px 16px;border-radius:20px;border:1px solid rgba(255,182,200,.5);background:rgba(0,0,0,.55);color:#fff;font-size:13px;cursor:pointer;font-family:"Microsoft YaHei",sans-serif';
ov.appendChild(quizCloseBtn);
// 三铁律注册即得(2026-07-28 深化⑤):✕/Esc 任何阶段可关;点外圈答题中(stage=quiz)拦截
const quizOvApi=ctx.overlay.register(ov,{
  x:'#quizCloseBtn',
  canClose(reason){return reason==='outside'?ov.dataset.stage!=='quiz':true;},
});

let sessionId=null,mcData=[],chosen=[],qaMinLen=25,quizTrack='';
let judgedQ={},rightCount=0; // 逐题批改:题号→已判,答对数(驱动金色裂纹)
...
async function beginQuiz(track){
  ...
  try{
    ...
    sessionId=d.sessionId;mcData=d.mc;chosen=new Array(d.mc.length).fill('');
    renderQuiz(d);
  }catch(e){
    box.innerHTML=`<div class="qz-spin">出卷失败:${e.message}<br><br><button class="qz-main-btn" onclick="startQuiz()">返回重试</button></div>`;
  }
}
...
<button class="qz-main-btn" id="qzSubmit" disabled>提交试卷</button>`;
  ...
  box.querySelectorAll('.qz-opt').forEach(btn=>{
    btn.addEventListener('click',async()=>{
      ...
      box.querySelectorAll(`.qz-opt[data-q="${q}"]`).forEach(b=>b.classList.toggle('sel',b===btn));
      checkReady();
      // 逐题批改:服务端判定,只回对错;答对一题,裂纹亮一丝
      try{
        const r=await fetch('/api/quiz/judge',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sessionId,qIndex:q,letter:btn.dataset.l})});
        const d=await r.json();
        if(!r.ok)throw 0;
        ...
        if(d.correctLetter){
          ...
          if(qBox){
            qBox.querySelectorAll('.qz-opt').forEach(b=>{if(b.dataset.l===d.correctLetter)b.classList.add('right');});
            if(d.explain){
              const ex=document.createElement('div');
              ex.style.cssText='font-size:12px;color:rgba(255,220,160,.85);line-height:1.7;margin-top:8px;border-top:1px dashed rgba(255,220,160,.3);padding-top:6px';
  ...
  // 禁止复制/粘贴/剪切(问答题必须手打)
  ['paste','copy','cut','drop','contextmenu'].forEach(ev=>ta.addEventListener(ev,e=>e.preventDefault()));
...
async function submitQuiz(){
  ...
  try{
    const r=await fetch('/api/quiz/submit',{
      ...
      body:JSON.stringify({sessionId,track:quizTrack,answers:chosen,qaText:(box.querySelector('#qzTa')&&box.querySelector('#qzTa').value)||window._qzTaVal||''}),
    });
    const d=await r.json();
    if(!r.ok)throw new Error(d.error||'提交失败');
    renderResult(d);
  }catch(e){
    box.innerHTML=`<div class="qz-spin">提交失败:${e.message}<br><br><button class="qz-main-btn" onclick="startQuiz()">重新挑战</button></div>`;
  }
}
...
function renderResult(d){
  ...
  const qaInfo=d.qaBy==='ai'
    ?`AI 阅卷评语:${escH(d.qaComment||'—')}`
    :(d.qaBreakdown||[]).map(b=>`${b.name} <b>${b.got}</b>/${b.max}<span style="opacity:.55"> (${escH(b.note)})</span>`).join('<br>');
...
<div class="qz-break">选择题 ${d.mcScore} / 81 分 · 问答题 ${d.qaScore} / 19 分<br>${qaInfo}</div>`;
  d.review.forEach((rv,i)=>{
    const mark=rv.right?'<span style="color:#7dff9a">✓ 答对</span>':'<span style="color:#ff8a8a">✗ 答错</span>';
    html+=`<div class="qz-q"><div class="qz-head">${QZ_TITLES[i]||('第 '+(i+1)+' 题')} · ${rv.subject} · ${mark}</div>`;
    if(rv.correctLetter)html+=`<div style="font-size:12px;color:rgba(255,220,160,.85);line-height:1.7;margin-top:4px">正解 ${rv.correctLetter}${rv.explain?' · '+escH(rv.explain):''}</div>`;
    html+='</div>';
  });
  // 神话卷:问答题附出题意图与参考方向
  if(d.qaExplain)html+=`<div class="qz-q" style="border:1px dashed rgba(255,214,130,.4)"><div class="qz-head">第十问 · 解析</div><div style="font-size:12px;color:rgba(255,220,160,.85);line-height:1.8">${escH(d.qaExplain)}</div></div>`;
  // 答题攒天穹(2026-07-26 主人新规则):答对一道选择题 +1(任何卷);问答题只要写了字 +1
  const gain=d.review.filter(r=>r.right).length+(((window._qzTaVal||'').trim().length>0)?1:0);
  if(gain>0){
    ...
    if(ctx.ui.kunlunSpeak&&(nq===1||Math.floor(prev/5)<Math.floor(nq/5)))ctx.ui.kunlunSpeak('灵蕴归位。');
  ...
  box.innerHTML=html;
  box.scrollTop=0;
  if(d.total===100)ctx.ui.kunlunSpeak&&ctx.ui.kunlunSpeak('十问皆通。欢迎你，藏梦人。'); // 满分:昆仑亲迎
  else if(d.passed)ctx.ui.kunlunSpeak&&ctx.ui.kunlunSpeak('十问过其六，灵蕴已生。推门进去吧。'); // ≥60:昆仑放行
  if(d.passed){
    unlockGallery(true);
    box.querySelector('#qzEnter').addEventListener('click',()=>{quizOvApi.close();});
  }else if(d.invite){
    const btn=box.querySelector('#qzInvite');
    if(btn)btn.addEventListener('click',async()=>{
      btn.disabled=true;btn.textContent='正在送达…';
      try{
        const r=await fetch('/api/quiz/invite',{method:'POST'});
        const dd=await r.json();
        if(!r.ok||!dd.passed)throw new Error(dd.error||'失败');
        unlockGallery(true);
        ctx.ui.kunlunSpeak&&ctx.ui.kunlunSpeak('十问未尽，灵蕴未满。但山不拒来者。进去看看吧。');
        box.innerHTML='<div class="qz-spin">🎉 邀请函已生效,欢迎进入画廊</div>';
        setTimeout(()=>{quizOvApi.close();},900);
      }catch(e){
        btn.disabled=false;btn.textContent='接 受 邀 请 函';
...
bag.custom.push(()=>{
  for(const b of gateBounds){const i=ctx.scene.bounds.indexOf(b);if(i>=0)ctx.scene.bounds.splice(i,1);}
  for(const m of [panelMesh,gateSign]){if(!m)continue;const i=iG.indexOf(m);if(i>=0)iG.splice(i,1);}
