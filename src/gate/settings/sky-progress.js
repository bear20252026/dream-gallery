...
// 2026-09-04:一念接入——每写一念=星屑+5(零门槛参与,与上传照片同权重)
function skyVal() {
...
const SKY_MS = [
  [5, '第一道裂痕的边缘，亮起了微光。', '第一道裂痕的边缘，亮起了微光。B612等这一刻，等了很久。'],
  [10, '天穹记住了你的声音。', '天穹记住了你的声音。你答对的每一道题，都在为它缝上第一针。'],
  [20, '第一缕天光，透了进来。', '第一缕天光，透了进来。你带来的星屑，已经开始修补这片天空。'],
  [35, 'B612的山风，忽然温柔了。', 'B612的山风，忽然温柔了。它认出了你——那个愿意凝视的人。'],
  [
    50,
    '裂痕收窄了一半。天穹记得你的每一幅画。',
  ...
  [
    80,
    '还差最后一块。你听见B612 的回音了吗？',
    '还差最后一块。你听见B612 的回音了吗？她说，B612 一直在等一个人，等一个愿意把记忆带回来的旅人。',
  ],
  [90, '天穹即将合拢。最后十步。', '天穹即将合拢。最后十步。你走过的每一步，B612都替你记得。'],
  [95, '再挂一幅。就一幅。', '再挂一幅。就一幅。那片天，在等你把它补完。'],
  [
    98,
    '你几乎能触到那片完整的天了。',
    '你几乎能触到那片完整的天了。它就在你眼前，只差最后一丝星屑。',
  ],
  [
    100,
    '天穹已合。你做到了。',
    '天穹已合。你做到了。你带来的每一片星屑，都回到了它该在的地方。你答过的每一道题，都是你对天地的回答。你挂过的每一幅画，都是人间留给B612的证明。三千年了。你是第一个走完这条路的人。但B612不闭门。新的裂痕总会生出——新的记忆也在等你。你愿意回来吗，藏梦人？',
  ],
];
...
function skyFullDialog() {
  ...
  d.querySelector('#sfStay').onclick = () => {
    sfApi.unregister();
    d.remove();
    ctx.ui.modeToast && ctx.ui.modeToast('B612谢过你。');
  };
  d.querySelector('#sfLeave').onclick = () => {
    sfApi.unregister();
    d.remove();
    ctx.ui.modeToast && ctx.ui.modeToast('B612留着你的光。');
  };
}
