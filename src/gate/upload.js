...
btn.id='upBtn';btn.style.display='none'; // 功能已并入 ⚙ 设置二级页(2026-07-25 主人定:悬浮按钮只留行走/跳跃类)
...
myLinkListEl.style.cssText='margin-top:10px;border-top:1px solid rgba(255,255,255,0.12);padding-top:8px';
...
document.getElementById('upX').onclick=function(){toggleUpPanel(false);};
btn.onclick=()=>toggleUpPanel();
ctx.mode.openUpload=function(){toggleUpPanel(true);}; // ⚙ 设置二级页入口
const $=id=>document.getElementById(id);
$('tabPhoto').onclick=()=>{$('tabPhoto').classList.add('on');$('tabLink').classList.remove('on');$('panePhoto').style.display='block';$('paneLink').style.display='none';};
$('tabLink').onclick=()=>{$('tabLink').classList.add('on');$('tabPhoto').classList.remove('on');$('paneLink').style.display='block';$('panePhoto').style.display='none';};
...
$('file').onchange=()=>{
  ...
  if(picked){
    ...
    if(picked.size>max){prog(isVid?'视频超过 700MB,换个小点的':'图片超过 50MB,换个小点的');picked=null;return;}
    $('drop').textContent='已选:'+picked.name+(isVid?'(视频)':'(图片)');
    $('doUp').style.display='block';
  }
};
function prog(t){$('upProg').textContent=t;}
setInterval(()=>{if(!open)prog('');},4000);

// ===== 空中悬浮路标:虚线从脚下延伸到目标,流光点循环向前,带你找到自己的作品 =====
function guideTo(target){
  if(!ctx.player.pl)return;
  const from=ctx.player.pl.p.clone();from.y=2.6;
  const to=new THREE.Vector3(target.x,(target.y||2.2)+1.2,target.z);
  const geo=new THREE.BufferGeometry().setFromPoints([from,to]);
  const mat=new THREE.LineDashedMaterial({color:'#ffd700',dashSize:0.5,gapSize:0.3,transparent:true,opacity:0.75});
  const line=new THREE.Line(geo,mat);
  line.computeLineDistances();
  s.add(line);
  // 终点金色箭头(向下指,自转+浮动)
  const arrow=new THREE.Mesh(new THREE.ConeGeometry(0.25,0.7,4),new THREE.MeshBasicMaterial({color:'#ffd700'}));
  ...
  const dots=[];
  for(let i=0;i<3;i++){
    const d=new THREE.Mesh(new THREE.SphereGeometry(0.09,8,8),new THREE.MeshBasicMaterial({color:'#ffee88'}));
    s.add(d);dots.push(d);
  }
  ...
  const cleanup=()=>{
    s.remove(line);s.remove(arrow);
    for(const d of dots)s.remove(d);
    const i=ctx.tickers.indexOf(tick);if(i>=0)ctx.tickers.splice(i,1);
  };
  ctx.onTick(tick);
...
function playUploadHint(onEnd){
  try{
    ...
    const paused=[];
    let done=false;
    const finish=()=>{if(done)return;done=true;while(paused.length){const m=paused.pop();m.play().catch(()=>{});}if(onEnd)onEnd();};
    const pauseOthers=()=>{
      const all=new Set(document.querySelectorAll('video,audio'));
      [ctx.media.vidEl,ctx.media.v45El,ctx.media.mA,ctx.kunlun.flyAudio,ctx.kunlun.peakVidEl].forEach(m=>{if(m)all.add(m);}); // 未挂 DOM 的也纳入
      all.forEach(m=>{if(m!==snd&&!m.paused){m.pause();paused.push(m);}});
    };
    snd.addEventListener('ended',finish);
    snd.addEventListener('error',finish);
    // 浏览器不放行(异步上传完成后手势已过期,部分移动端)就不播,也不打扰其他媒体;同样走 onEnd 让 TTS 接上
...
function goldDust(){
  ...
  for(let i=0;i<26;i++){
    ...
    box.appendChild(p);
    const dx=(Math.random()-0.5)*120,dy=-(120+Math.random()*220);
    p.animate([
...
const AURA_KWS={生机:['花','春','绿','阳','海','笑','草','晴','蓝','光'],炽烈:['红','爱','热','心动','泪','焰','醉','舞','甜'],萧瑟:['暮','黄昏','雨','雪','夜','别','旧','寂','凉','秋'],安宁:['静','书','猫','家','茶','灯','独','安','眠','午后']};
const AURA_PLACE={生机:'东墙·生机位',炽烈:'南墙·炽烈位',萧瑟:'西墙·萧瑟位',安宁:'北墙·安宁位'};
function classifyAura(t){if(!t)return null;let best=null,bn=0;for(const[k,ws] of Object.entries(AURA_KWS)){let n=0;for(const w of ws)if(t.includes(w))n++;if(n>bn){bn=n;best=k;}}return best;}
$('doUp').onclick=async()=>{
  if(!picked)return;
  const file=picked;
  ...
  if(!isVid){
    try{
      ...
      if(set.includes(hex))dup=true;else{set.push(hex);ctx.store.setJson('upHash',set);}
    }catch(e){}
  }
  try{
    ...
    picked=null;$('drop').textContent='点这里选择照片/视频';$('doUp').style.display='none';
    // AI 配文(仅图片;失败用回退句,不阻塞上墙)
    let caption=isVid?'访客上传的视频':'访客上传的照片';
    if(!isVid){
      try{
        const rv=await fetch('/api/vision/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({file:name})});
    ...
    if(!isVid){
      if(dup){ctx.ui.modeToast&&ctx.ui.modeToast('这片灵蕴已归过位，不再重复计算。');}
      else{
        const u=ctx.store.num('up')+1;ctx.store.setNum('up',u);
        ctx.ui.modeToast&&ctx.ui.modeToast('灵蕴 +5。天穹裂痕收窄了一寸。');
        ctx.kunlun.checkSkyMs&&ctx.kunlun.checkSkyMs();
      }
    }
    open=false;panel.classList.remove('show');
  }catch(e){prog(e.message||'上传失败,请稍后再试');}
};
...
let pickedModel='sphere';
{
  ...
  const PRESETS=[['百度','https://www.baidu.com'],['哔哩哔哩','https://www.bilibili.com'],['王者营地','https://pvp.qq.com/cp/a20190918confrontationm/index.html'],['爱奇艺','https://mbd.baidu.com/ug_share/mbox/4a83aa9e65/share?product=external&tk=50777bfaa57553a1ae74eafb54e8d53e&share_url=https%3A%2F%2Fm.iqiyi.com%2F&domain=mbd.baidu.com']];
  const pw=$('lkPresets');
  for(const [n,u] of PRESETS){
    const b=document.createElement('button');
    b.textContent=n;
    b.onclick=()=>{$('lkName').value=n;$('lkUrl').value=u;};
    pw.appendChild(b);
  }
  const wrap=$('lkModels');
  ...
  Object.keys(types).forEach((k,i)=>{
    ...
    b.onclick=()=>{pickedModel=k;wrap.querySelectorAll('button').forEach(x=>x.classList.remove('on'));b.classList.add('on');};
    wrap.appendChild(b);
  });
}
$('doLk').onclick=async()=>{
  const name=$('lkName').value.trim(),url=$('lkUrl').value.trim();
  if(!name||!/^https?:\/\//i.test(url)){prog('名称和 http(s) 链接都要填对');return;}
  // 模型出现在眼前:玩家前方 3 米
  const pos={x:0,y:2.0,z:0};
  if(ctx.player.pl){
    const fx=-Math.sin(ctx.player.pl.y),fz=-Math.cos(ctx.player.pl.y);
    pos.x=+(ctx.player.pl.p.x+fx*3).toFixed(1);pos.z=+(ctx.player.pl.p.z+fz*3).toFixed(1);
  }
  prog('落成中…');
  try{
    const r=await fetch('/api/mylinks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'add',name,url,model:pickedModel,pos})});
    const d=await r.json();
    ...
    renderMyLinks();
    guideTo(pos);
    setTimeout(()=>{open=false;panel.classList.remove('show');},800);
  }catch(e){prog(e.message||'失败,请稍后再试');}
};

// 我的链接管理列表(含删除)
async function renderMyLinks(){
  const list=ctx.mode.myLinks||[];
  myLinkListEl.innerHTML=list.length?list.map((l,i)=>`<div style="display:flex;align-items:center;gap:6px;padding:5px 0;font-size:12px">
  ...
  </div>`).join(''):'<div style="font-size:11px;opacity:.5">还没有自己的链接</div>';
  myLinkListEl.querySelectorAll('button[data-i]').forEach(b=>{
    b.onclick=async()=>{
      const l=list[+b.dataset.i];
      b.disabled=true;b.textContent='…';
      try{
        const r=await fetch('/api/mylinks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'del',id:l.id})});
        if(!r.ok)throw 0;
        if(ctx.mode.refreshMode)await ctx.mode.refreshMode();
        renderMyLinks();
      }catch(e){b.disabled=false;b.textContent='删除';}
    };
  });
}
