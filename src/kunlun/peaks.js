// peaks.js — 远方山巅巅彩蛋(2026-07-25 主人定)
...
function holdBigScreen(on) {
  ...
  for (const el of [ctx.media.vidEl, ctx.media.v45El]) {
    ...
    if (on) {
      ...
      if (avAllowed() && el.paused && el.readyState >= 2) el.play().catch(function () {});
    }
  }
}
...
flyAudio.addEventListener('ended', function () {
  holdBigScreen(false);
});
flyAudio.addEventListener('error', function () {
  holdBigScreen(false);
});
