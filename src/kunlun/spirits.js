...
// ===================== 六灵蕴数据(文案=主人终版) =====================
const KX=800,KZ=600; // 昆仑坐标(与 desert.js 一致)
const SPIRITS=[
  {key:'sprout',name:'春生之芽',en:'Sprout of Spring',emotion:'希望',color:'#7ddb7a',pos:[KX-150,KZ],place:'昆仑东麓·初阳坡',
   popup:'你找到了「春生之芽」。',
   tts:'春生之芽。它藏在昆仑最早照到阳光的那道石缝里。你曾在无数个清晨醒来，却很少留意窗台上那盆植物长出了第几片新叶。但希望从来不需要被注视——它只管生长。就像你决定上传第一张照片时那样。'},
  {key:'flame',name:'夏炽之焰',en:'Flame of Summer',emotion:'热爱',color:'#ff5a4a',pos:[KX-60,KZ-20],place:'昆仑之巅·望天石下',
   popup:'你找到了「夏炽之焰」。',
   tts:'夏炽之焰。它沉睡在昆仑最高处的一块岩石之下。你或许已经忘了那个夏天——汗流浃背却不肯放下相机的那天。但昆仑记得。记得你胸口那团滚烫的、想把一切都记录下来的冲动。那就是灵蕴最初的形状。'},
  {key:'leaf',name:'秋思之叶',en:'Leaf of Autumn',emotion:'眷恋',color:'#e8a03c',pos:[KX+80,KZ-140],place:'昆仑北麓·枯草坡',
   popup:'你找到了「秋思之叶」。',
   tts:'秋思之叶。它飘落在昆仑北坡的一片枯草地上。你大概已经不太记得，最后一次见到那个人是什么季节。但每一次秋风吹起来的时候，你心里都会泛起同样的涟漪。眷恋是另一种记忆——它不用照片，只用心跳。'},
  {key:'snow',name:'冬藏之雪',en:'Snow of Winter',emotion:'沉静',color:'#dfeaf5',pos:[KX-60,KZ+170],place:'昆仑深处·暗河源头',
   popup:'你找到了「冬藏之雪」。',
   tts:'冬藏之雪。它沉在昆仑深处一条暗河的源头。你生命中有过一些时刻，什么话都不想说，什么人都不愿见。但那些沉默并不是空白——它们是雪的质地。安静、缓慢、却厚重。等你回头再碰它时，已经化成了水，滋养了下一季。'},
  {key:'dawn',name:'朝露之珠',en:'Bead of Dawn',emotion:'新生',color:'#7cc8e8',pos:[KX-110,KZ-80],place:'昆仑东崖·草叶尖端',
   popup:'你找到了「朝露之珠」。',
   tts:'朝露之珠。它凝结在昆仑东崖的一片草叶尖端。黎明前的光总是最轻的。你那些重新开始的念头——换了新工作、搬了新城市、删了旧照片——都像朝露。它们只出现一次，只在最安静的时刻。但你抓住了。'},
  {key:'dusk',name:'暮光之尘',en:'Dust of Dusk',emotion:'释然',color:'#f0a860',pos:[KX+170,KZ+60],place:'昆仑西麓·霞光台',
   popup:'你找到了「暮光之尘」。',
   tts:'暮光之尘。它悬浮在昆仑西侧的一片晚霞里。你曾经有多少次看见夕阳，却没有停下来看它？那些错过的黄昏都变成了尘埃——飘在空中，等着有人抬头。而你终于抬头了。这就是最后一枚灵蕴。它告诉你：所有未完成的，都可以在日暮时被原谅。'},
];
const INTRO_TTS='天可补，心难全。昆仑之下，六合之间，散落着六枚心象灵蕴。它们藏于昆仑的六个角落，每一枚都对应一种凡人最珍贵的情感。寻回六枚，万镜画廊的真正面目，才会向你打开。';
const FINAL_POPUP='六灵蕴齐聚。万镜画廊的门，重新打开了。';
const FINAL_TTS='六灵蕴齐聚。你已穿越昆仑的春夏秋冬，拾回了希望、热爱、眷恋、沉静、新生与释然。你以为你一直在补天。其实，你一直在补自己。从今往后，愿你继续记住。';
...
function gotKeys(){return ctx.store.getSpirits();}
function got(){return gotKeys().length;}
function isDone(){return got()>=6;}
function nextIdx(){const k=gotKeys();return SPIRITS.findIndex(sp=>!k.includes(sp.key));} // 首个未收集=当前揭示目标
...
function pillarTexture(){
  ...
  g.addColorStop(0,'rgba(255,214,130,0.9)');g.addColorStop(0.7,'rgba(255,214,130,0.35)');g.addColorStop(1,'rgba(255,214,130,0)');
  x.fillStyle=g;x.fillRect(0,0,16,128);
  return new THREE.CanvasTexture(c);
}
let pillar=null,pillarRing=null,pickBtn=null;
// 指引 HUD(2026-07-27,参考 Level 37 的 q-arrow 模式):屏顶旋转箭头 + 名字/地点/距离
let hud=null,hudArrow=null,hudText=null;
// 悬浮名牌(设计文档:近距离光柱顶部浮现灵蕴名称;这里 60m 内常显,远近都看得见目标)
let nameSpr=null,nameCanvas=null,nameCtx=null,nameTex=null;
...
function buildPillar(){
  ...
  const mat=new THREE.MeshBasicMaterial({map:tex,transparent:true,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending});
  pillar=new THREE.Mesh(new THREE.CylinderGeometry(0.9,1.2,14,12,1,true),mat);
  pillarRing=new THREE.Mesh(new THREE.RingGeometry(1.2,1.6,40),new THREE.MeshBasicMaterial({color:'#ffd88a',transparent:true,opacity:0.6,side:THREE.DoubleSide,depthWrite:false}));
  pillarRing.rotation.x=-Math.PI/2;
  s.add(pillar);s.add(pillarRing);
  // 悬浮名牌(3D 世界内,贴在光柱上方)
  ...
  nameSpr.scale.set(7,1.75,1);nameSpr.visible=false;s.add(nameSpr); // 可见性由主循环按距离控制
  // 指引 HUD(DOM,屏顶居中,纯展示不拦截触摸)
  hud=document.createElement('div');
  hud.style.cssText='position:fixed;top:52px;left:50%;transform:translateX(-50%);z-index:55;display:none;flex-direction:column;align-items:center;pointer-events:none;font-family:inherit';
  hudArrow=document.createElement('div');
  hudArrow.textContent='▲';
  hudArrow.style.cssText='font-size:18px;line-height:20px;color:#ffd88a;text-shadow:0 0 10px rgba(255,200,100,.85),0 1px 4px rgba(0,0,0,.8)';
  hudText=document.createElement('div');
  hudText.style.cssText='margin-top:2px;font-size:11px;letter-spacing:2px;color:rgba(255,232,190,.9);text-shadow:0 1px 4px rgba(0,0,0,.75);white-space:nowrap';
  hud.appendChild(hudArrow);hud.appendChild(hudText);
  document.body.appendChild(hud);
  // 拾取按钮(走近才出现)
  pickBtn=document.createElement('button');
  pickBtn.textContent='拾 取';
  pickBtn.style.cssText='position:fixed;left:50%;bottom:160px;transform:translateX(-50%);z-index:60;display:none;padding:12px 30px;border-radius:24px;border:1px solid rgba(255,214,130,.7);background:rgba(40,26,12,.8);color:#ffe9c4;font-size:16px;letter-spacing:4px;cursor:pointer;font-family:inherit';
  ...
  const wctx2=wcv.getContext('2d');const wg=wctx2.createRadialGradient(32,32,2,32,32,30);
  wg.addColorStop(0,'rgba(255,255,255,0.9)');wg.addColorStop(1,'rgba(255,255,255,0)');
  wctx2.fillStyle=wg;wctx2.fillRect(0,0,64,64);
  wildSpr=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(wcv),transparent:true,depthWrite:false,blending:THREE.AdditiveBlending}));
  wildSpr.scale.set(3,3,1);wildSpr.visible=false;s.add(wildSpr);
}
function drawName(i){
  const sp=SPIRITS[i],x=nameCtx;
  x.clearRect(0,0,256,64);
  x.font='bold 30px KaiTi,serif';x.textAlign='center';x.textBaseline='middle';
  x.shadowColor='rgba(0,0,0,.9)';x.shadowBlur=8;
  x.fillStyle=sp.color;x.fillText('✦ '+sp.name+' ✦',128,32);
...
function placePillar(i){
  ...
  if(ctx.media.desert.assertAboveGround)ctx.media.desert.assertAboveGround(sp.pos[0],y+0.15,sp.pos[1],'灵蕴·'+sp.name);
  ...
  pillar.material.color=col;pillarRing.material.color=col;
...
function collect(){
  ...
  if(ti<0)return;
  collecting=true;pickIdx=-1;
  pickBtn.style.display='none';
  pillar.visible=pillarRing.visible=false;
  nameSpr.visible=false;if(wildSpr)wildSpr.visible=false;
  const sp=SPIRITS[ti];
...
function finale(){
  ...
  for(const sp of SPIRITS){
    const y=groundY(sp.pos[0],sp.pos[1]);
    const b=new THREE.Mesh(new THREE.CylinderGeometry(0.5,1.0,60,10,1,true),
      new THREE.MeshBasicMaterial({map:pillarTexture(),color:sp.color,transparent:true,opacity:0.9,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending}));
    b.position.set(sp.pos[0],y+30,sp.pos[1]);s.add(b);beams.push(b);
  }
...
// ===================== 主循环:激活检测 + 走近提示 + 呼吸动画 =====================
let started=false,nearT=0,hudT=0;
onTick(function(){
  ...
  if(got()>=6||!pillar.visible){pickBtn.style.display='none';hud.style.display='none';nameSpr.visible=false;if(wildSpr)wildSpr.visible=false;return;}
  // 呼吸:光柱明暗+底座环旋转
  const t=performance.now()*0.001;
  pillar.material.opacity=0.75+Math.sin(t*2)*0.25;
  pillarRing.rotation.z+=0.01;
  if(!ctx.player.pl)return;
  const ti=nextIdx();if(ti<0)return;
  // 指引 HUD + 悬浮名牌(8Hz 节流,与参考实现一致;飞行/展厅内不打扰)
  if(t-hudT>0.12){
    hudT=t;
    const sp=SPIRITS[ti];
    const hx=sp.pos[0]-ctx.player.pl.p.x,hz=sp.pos[1]-ctx.player.pl.p.z;
    const dist=Math.hypot(hx,hz);
    const tAng=Math.atan2(hx,hz),fAng=Math.atan2(-Math.sin(ctx.player.pl.y),-Math.cos(ctx.player.pl.y));
    let rel=tAng-fAng;
  ...
  for(let j=0;j<6;j++){
    ...
    const ddx=ctx.player.pl.p.x-SPIRITS[j].pos[0],ddz=ctx.player.pl.p.z-SPIRITS[j].pos[1];
    if(ddx*ddx+ddz*ddz<9){pi=j;break;}
  }
  if(pi>=0){pickIdx=pi;if(!nearT){pickBtn.style.display='block';pickBtn.textContent='拾 取 · '+SPIRITS[pi].name;}}
  else{pickIdx=-1;pickBtn.style.display='none';}
  nearT=pi>=0?1:0;
  // 野灵感应微光:25m 内最近的未揭示灵蕴浮现柔光团(不剧透名字,只提示"这里有东西")
  let wi=-1,wd=25;
  for(let j=0;j<6;j++){
    if(j===ti||keys.includes(SPIRITS[j].key))continue;
    const d=Math.hypot(SPIRITS[j].pos[0]-ctx.player.pl.p.x,SPIRITS[j].pos[1]-ctx.player.pl.p.z);
    if(d<wd){wd=d;wi=j;}
  }
  if(wildSpr){
    if(wi>=0&&!ctx.kunlun.flightLock){
      wildSpr.position.set(SPIRITS[wi].pos[0],groundY(SPIRITS[wi].pos[0],SPIRITS[wi].pos[1])+1.5,SPIRITS[wi].pos[1]);
      wildSpr.material.color.set(SPIRITS[wi].color);
      const pu=2.4+Math.sin(t*3)*0.5;wildSpr.scale.set(pu,pu,1);
...
function spiritsState(){
  const keys=gotKeys(),ti=nextIdx();
  return SPIRITS.map((sp,i)=>({name:sp.name,en:sp.en,emotion:sp.emotion,color:sp.color,place:sp.place,
...
buildPillar();
Object.assign(ctx.kunlun,{spiritsState,isDone:()=>isDone(),spiritsGot:got,spiritsTTS:SPIRITS.map(sp=>sp.tts),spiritMark}); // spiritsTTS 供 finale.js 灵蕴归位重听
bag.custom.push(()=>{if(pickBtn)pickBtn.remove();if(hud)hud.remove();});
hotEnd('spirits');
