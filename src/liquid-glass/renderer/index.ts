...
export class LiquidGlassRenderer {
  ...
  /** Transparent-overlay mode: the canvas is composited with alpha over the
   *  page (used when integrating as a transparent glass layer above an
   *  existing Three.js scene). When true, render() clears fboA and the
   *  default framebuffer to transparent instead of drawing an opaque
   *  wallpaper, and glass elements should set `sampleWallpaper` so their
   *  refraction samples the backdrop canvas (the live 3D scene) rather than
   *  the (now transparent) scene FBO. */
  transparent = false
  /** Live backdrop canvas (e.g. a Three.js renderer.domElement) sampled as
   *  the wallpaper each frame via updateBackdrop(). */
  backdropCanvas: HTMLCanvasElement | null = null
  ...
  constructor(canvas: HTMLCanvasElement, opts?: { transparent?: boolean }) {
    this.canvas = canvas
