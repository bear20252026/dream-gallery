...
export const renderMethods = {
  render(this: LiquidGlassRenderer) {
    ...
    if (this.transparent) {
      // Transparent-overlay mode: do NOT paint an opaque wallpaper. Clear
      // the scene FBO to transparent so non-glass regions of the final
      // image are see-through (the underlying page / Three.js canvas shows).
      // Glass elements opt into `sampleWallpaper` so their refraction reads
      // the live backdrop canvas (the 3D scene) instead of this empty FBO.
