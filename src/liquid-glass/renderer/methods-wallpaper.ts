...
declare module './index' {
  interface LiquidGlassRenderer {
    ...
    /** Use a live canvas (e.g. a Three.js renderer.domElement) as the
     *  wallpaper/backdrop. The host calls updateBackdrop() each frame so
     *  glass elements (with sampleWallpaper) refract the live scene. */
    setBackdropCanvas(canvas: HTMLCanvasElement): void
    /** Re-upload the backdrop canvas into wallpaperTexture. Call every frame
     *  before render() when the backdrop is a live (animated) canvas. */
    updateBackdrop(): void
  }
}
...
export const wallpaperMethods = {
  ...
  /** Use a live canvas as the wallpaper/backdrop (transparent-overlay mode).
   *  Creates wallpaperTexture if needed and uploads one frame immediately. */
  setBackdropCanvas(this: LiquidGlassRenderer, canvas: HTMLCanvasElement) {
    this.backdropCanvas = canvas
    if (!this.wallpaperTexture) {
      this.wallpaperTexture = this.gl.createTexture()!
    }
    this.updateBackdrop()
  },

  /** Re-upload the backdrop canvas (a live Three.js render) into
   *  wallpaperTexture so glass elements refract the current frame.
   *  The host calls this every frame before render(). */
  updateBackdrop(this: LiquidGlassRenderer) {
    const cv = this.backdropCanvas
    if (!cv) return
    const gl = this.gl
    if (!this.wallpaperTexture) this.wallpaperTexture = gl.createTexture()!
    gl.bindTexture(gl.TEXTURE_2D, this.wallpaperTexture)
    // Canvas origin is top-left; WebGL textures are bottom-left by default,
    // so flip Y so the 3D scene appears upright in the glass refraction.
    gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true)
    ...
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE)
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE)
    this.wallpaperSize = [cv.width || 1, cv.height || 1]
    this.wallpaperReady = true
  },
}
