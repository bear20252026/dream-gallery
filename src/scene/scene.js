...
const s=new THREE.Scene();s.background=null; // 天空球提供背景
s.fog=new THREE.FogExp2('#C8B88A',0.006); // 指数雾(密度对齐西域线性雾手感;挂画放大景深虚化依赖 density)
const rnd=new THREE.WebGLRenderer({antialias:true});rnd.setPixelRatio(Math.min(devicePixelRatio,2));rnd.setSize(innerWidth,innerHeight);rnd.outputColorSpace=THREE.SRGBColorSpace;C.appendChild(rnd.domElement);
...
rnd.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();location.reload();});
const cam=new THREE.PerspectiveCamera(75,innerWidth/innerHeight,0.1,2000);cam.rotation.order='YXZ';
...
const pendingTex=[];let texGateTimer=null;
// 距离懒加载(2026-07-25):带坐标的纹理只在玩家 35m 内加载,弱网/弱 GPU 省首屏
const NEAR_D2=35*35;
const nearPending=[];let nearTimer=null;
function loadTexCapped(url,onErr,pos){
  const cv=document.createElement('canvas');cv.width=cv.height=TEX_CAP;
  const cx2=cv.getContext('2d');cx2.fillStyle='#e8e0e4';cx2.fillRect(0,0,TEX_CAP,TEX_CAP);
  const tex=new THREE.CanvasTexture(cv);tex.colorSpace=THREE.SRGBColorSpace;
  tex.minFilter=THREE.LinearMipmapLinearFilter;
  let tries=0,queued=false,noThumb=false;
  // 缩略图优先(gen-thumbs.js 预生成 photos/thumbs/*.webp),404 回退原图
  const thumbUrl=/^photos\//.test(url)&&!/\.webp$/i.test(url)?url.replace(/\.[^.]+$/,'.webp').replace(/^photos\//,'photos/thumbs/'):url;
  ...
  function tok(u){const t=(ctx.mode.myUploadTokens||{})[u.split('/').pop().split('?')[0]];return t?u+'?mt='+t:u;}
  function attempt(){
    // 距离门禁:给了坐标且玩家还远,挂进距离队列,走近再加载
    if(pos&&ctx.player.pl){
      const dx=pos.x-ctx.player.pl.p.x,dz=pos.z-ctx.player.pl.p.z;
      if(dx*dx+dz*dz>NEAR_D2){
        if(!nearPending.includes(attempt))nearPending.push(attempt);
        if(!nearTimer)nearTimer=setInterval(()=>{
          for(let i=nearPending.length-1;i>=0;i--){const f=nearPending.splice(i,1)[0];f();}
          if(!nearPending.length){clearInterval(nearTimer);nearTimer=null;}
        },1500);
        return;
      }
    }
    ...
    if(!ctx.player.quizPassed||(ctx.mode.texAllowed&&!ctx.mode.texAllowed(url))){
      if(!queued){queued=true;pendingTex.push(attempt);}
      if(!texGateTimer)texGateTimer=setInterval(()=>{
        if(!ctx.player.quizPassed)return;
        // 全部重试:每个 attempt 自己再过一遍门禁,放行的加载,仍拦的自己重排队重武装
        // (曾按创建者的 url 判定,图库照片永久拦 → 整个队列卡死,2026-07-25 血泪)
        clearInterval(texGateTimer);texGateTimer=null;
    ...
    img.onerror=function(){
      ...
      if(!noThumb&&thumbUrl!==url){noThumb=true;img.src=tok(url);return;}
...
const OL=-18,OR=18,OT=-12,OBE=6;     // 展厅区边界 (E=exhibition)
...
function w(x1,z1,x2,z2){
  const dx=x2-x1,dz=z2-z1,len=Math.sqrt(dx*dx+dz*dz);if(len<0.3)return;
  const cx=(x1+x2)/2,cz=(z1+z2)/2,ang=Math.atan2(dx,dz);
  const hue=335+Math.random()*20;
  const wallMat=new THREE.MeshStandardMaterial({color:new THREE.Color(`hsl(${hue},45%,70%)`),roughness:0.92,metalness:0.02});
  houseMats.wall.push(wallMat); // 房屋换色(housecolor.js)分组染色
  const wall=new THREE.Mesh(new THREE.BoxGeometry(0.3,WH,len),wallMat);
  wall.position.set(cx,WH/2,cz);wall.rotation.y=ang;s.add(wall);
  const sA=Math.abs(Math.sin(ang)),cA=Math.abs(Math.cos(ang));
  bounds.push({mnX:cx-(sA*len/2+cA*0.2)-0.1,mxX:cx+(sA*len/2+cA*0.2)+0.1,mnZ:cz-(cA*len/2+sA*0.2)-0.1,mxZ:cz+(cA*len/2+sA*0.2)+0.1});
  ...
  bm1.position.set(cx+pX*0.2,0.05,cz+pZ*0.2);bm1.rotation.y=ang;s.add(bm1);
  // 外侧（场景外方向）
  const bm2=new THREE.Mesh(new THREE.BoxGeometry(0.1,0.1,len),baseM);
  bm2.userData={isBaseboard:true}; // 标记为踢脚线，挂画系统排除
  bm2.position.set(cx-pX*0.2,0.05,cz-pZ*0.2);bm2.rotation.y=ang;s.add(bm2);
}

// ===== 外围（覆盖整体范围） =====
// 北墙 z=-12, x=-18~18 (36m) → 14段
w(-18,-12,-15.5,-12);w(-15.5,-12,-13,-12);w(-13,-12,-10.5,-12);w(-10.5,-12,-8,-12);w(-8,-12,-5.5,-12);w(-5.5,-12,-3,-12);w(-3,-12,-0.5,-12);w(-0.5,-12,2,-12);w(2,-12,4.5,-12);w(4.5,-12,7,-12);w(7,-12,9.5,-12);w(9.5,-12,12,-12);w(12,-12,14.5,-12);w(14.5,-12,18,-12);
// 东墙 x=18, z=-12~28 (40m) → 16段
w(18,-12,18,-9.5);w(18,-9.5,18,-7);w(18,-7,18,-4.5);//w(18,-4.5,18,-2);//门洞z∈[-4.5,-2]w(18,-2,18,0.5);w(18,0.5,18,3);w(18,3,18,5.5);w(18,5.5,18,8);w(18,8,18,10.5);w(18,10.5,18,13);w(18,13,18,15.5);w(18,15.5,18,18);w(18,18,18,20.5);w(18,20.5,18,23);w(18,23,18,25.5);w(18,25.5,18,28);
//门洞装饰(x=18,z∈[-4.5,-2])
const df1=new THREE.Mesh(new THREE.BoxGeometry(0.12,2.5,0.12),decoM);df1.position.set(18,1.25,-4.5);s.add(df1);
const df2=new THREE.Mesh(new THREE.BoxGeometry(0.12,2.5,0.12),decoM);df2.position.set(18,1.25,-2);s.add(df2);
const df3=new THREE.Mesh(new THREE.BoxGeometry(0.15,WH-2.5,2.5),decoM);df3.position.set(18,(WH+2.5)/2,-3.25);s.add(df3);
const df4=new THREE.Mesh(new THREE.BoxGeometry(0.15,0.1,2.5),decoM);df4.position.set(18,2.5,-3.25);s.add(df4);
// 南墙 z=28, x=-18~18 (36m) → 14段
w(-18,28,-15.5,28);w(-15.5,28,-13,28);w(-13,28,-10.5,28);w(-10.5,28,-8,28);w(-8,28,-5.5,28);w(-5.5,28,-3,28);w(-3,28,-0.5,28);w(-0.5,28,2,28);w(2,28,4.5,28);w(4.5,28,7,28);w(7,28,9.5,28);w(9.5,28,12,28);w(12,28,14.5,28);w(14.5,28,18,28);
// 西墙 x=-18, z=-12~28 (40m) → 16段
w(-18,-12,-18,-9.5);w(-18,-9.5,-18,-7);w(-18,-7,-18,-4.5);w(-18,-4.5,-18,-2);w(-18,-2,-18,0.5);w(-18,0.5,-18,3);w(-18,3,-18,5.5);w(-18,5.5,-18,8);w(-18,8,-18,10.5);w(-18,10.5,-18,13);w(-18,13,-18,15.5);w(-18,15.5,-18,18);w(-18,18,-18,20.5);w(-18,20.5,-18,23);w(-18,23,-18,25.5);w(-18,25.5,-18,28);

// ===== 原展厅A-G（版本27布局，长墙分割）=====
// A左上 z=-12~-6
w(-18,-6,-15.5,-6);w(-15.5,-6,-13,-6);w(-13,-6,-10.5,-6);w(-10.5,-6,-8,-6);
w(-5.5,-6,-4,-6);
w(-4,-12,-4,-9.5);w(-4,-9.5,-4,-8);
// B右上 z=-12~-6
w(4,-6,5.5,-6);
w(8,-6,10.5,-6);w(10.5,-6,13,-6);w(13,-6,15.5,-6);w(15.5,-6,18,-6);
w(4,-12,4,-9.5);w(4,-9.5,4,-8);
// C左中 z=-6~0
w(-18,0,-15.5,0);w(-15.5,0,-13,0);w(-13,0,-10.5,0);w(-10.5,0,-8,0);
w(-5.5,0,-4,0);
w(-4,-6,-4,-4);w(-4,-4,-4,-2);w(-4,-2,-4,0);
// D右中 z=-6~0
w(6,0,8.5,0);w(8.5,0,11,0);w(11,0,13.5,0);w(13.5,0,18,0);
w(4,-6,4,-4);w(4,-4,4,-2);w(4,-2,4,0);
// E中下 z=0~6
w(-4,6,-1.5,6);w(1.5,6,4,6);  // 门洞 x∈[-1.5,1.5]
w(-4,4,-4,4.5);w(-4,5.5,-4,6);
w(4,4,4,4.5);w(4,5.5,4,6);
// F左下 z=6~12
w(-18,6,-10,6);w(-8,6,-4,6);  // 门洞 x∈[-10,-8]
w(-4,6,-4,8.5);w(-4,8.5,-4,10);w(-4,11,-4,12);
// G右下 z=6~12
w(4,6,8,6);w(10,6,18,6);  // 门洞 x∈[8,10]
w(4,6,4,8.5);w(4,8.5,4,10);w(4,11,4,12);
// 展示台
w(-1.5,8,1.5,8);
...
// 内北墙（z=11），北门洞 x∈[-2,2]
w(-7,11,-4.5,11);w(-4.5,11,-2,11);
w(2,11,4.5,11);w(4.5,11,7,11);
// 内南墙（z=23），南门洞 x∈[-2,2]
w(-7,23,-4.5,23);w(-4.5,23,-2,23);
w(2,23,4.5,23);w(4.5,23,7,23);
// 内西墙（x=-7），西门洞 z∈[15.5,18.5]
w(-7,11,-7,13.5);w(-7,13.5,-7,15.5);
w(-7,18.5,-7,21);w(-7,21,-7,23);
// 内东墙（x=7），东门洞 z∈[15.5,18.5]
w(7,11,7,13.5);w(7,13.5,7,15.5);
w(7,18.5,7,21);w(7,21,7,23);

// ===== 墙壁圆润化装饰（整边圆角管包裹）=====
const edgeTubeMat=new THREE.MeshStandardMaterial({color:'#c07080',roughness:0.4,metalness:0.2});
wI.forEach(wi=>{
  const pX=Math.cos(wi.ang),pZ=-Math.sin(wi.ang); // 法线（正面方向）
  const tX=Math.sin(wi.ang),tZ=Math.cos(wi.ang);  // 切线（沿墙方向）
  const off=0.17; // 管中心到墙表面的距离
  // 正面四根管状圆角条
  const tubeR=0.07,seg=6;
  // 顶边管（沿墙长度，在顶部）
  const topT=new THREE.Mesh(new THREE.CylinderGeometry(tubeR,tubeR,wi.len,seg),edgeTubeMat);
  topT.rotation.x=Math.PI/2;topT.rotation.z=wi.ang;
  topT.position.set(wi.cx+pX*off,WH-0.05,wi.cz+pZ*off);s.add(topT);
  // 底边管（沿墙长度，在底部/地板处）
  const botT=new THREE.Mesh(new THREE.CylinderGeometry(tubeR,tubeR,wi.len,seg),edgeTubeMat);
  botT.rotation.x=Math.PI/2;botT.rotation.z=wi.ang;
  botT.position.set(wi.cx+pX*off,0.05,wi.cz+pZ*off);s.add(botT);
  // 左边管（沿墙高度，在墙起始端）
  const lT=new THREE.Mesh(new THREE.CylinderGeometry(tubeR,tubeR,WH,seg),edgeTubeMat);
  lT.position.set(wi.x1+tX*0.15+pX*off,WH/2,wi.z1+tZ*0.15+pZ*off);s.add(lT);
  // 右边管（沿墙高度，在墙结束端）
  const rT=new THREE.Mesh(new THREE.CylinderGeometry(tubeR,tubeR,WH,seg),edgeTubeMat);
  rT.position.set(wi.x2-tX*0.15+pX*off,WH/2,wi.z2-tZ*0.15+pZ*off);s.add(rT);
});
...
const roofMat=new THREE.MeshStandardMaterial({color:'#e8e0d5',roughness:0.85,side:THREE.DoubleSide});
const roof=new THREE.Mesh(new THREE.PlaneGeometry(floorW,floorD),roofMat);
roof.rotation.x=Math.PI/2;roof.position.y=WH;roof.position.z=(OT+OBR)/2;s.add(roof);
...
roofThick.position.y=WH+0.08;roofThick.position.z=(OT+OBR)/2;s.add(roofThick);
...
const kintsugiTex=(function(){
  ...
  for(let v=0;v<9;v++){ // 9 条主纹,随机游走+断枝,如金缮修补痕
    let px=rnd()*512,py=rnd()*512,a=rnd()*Math.PI*2;
    x.strokeStyle='rgba(212,168,75,0.85)';x.lineWidth=1.6+rnd()*1.4;
    x.beginPath();x.moveTo(px,py);
    for(let k=0;k<14;k++){
      a+=(rnd()-0.5)*1.1;px+=Math.cos(a)*(18+rnd()*30);py+=Math.sin(a)*(18+rnd()*30);
      x.lineTo(px,py);
...
let kintsugiOn=false,kintsugiMat=null;
function enableKintsugi(){
  if(kintsugiOn)return;kintsugiOn=true;
  kintsugiMat=new THREE.MeshBasicMaterial({map:kintsugiTex,transparent:true,opacity:0.55,side:THREE.DoubleSide,depthWrite:false});
...
const bW=OR-OL,bD=OBR-OT; // 建筑宽36，深40
...
const pyrMat=new THREE.MeshStandardMaterial({color:'#ff88aa',transparent:true,opacity:0.55,roughness:0.4,metalness:0.1,side:THREE.DoubleSide});
...
s.add(pyramid);
// 发光钻石 - 在金字塔顶点
const diamond=new THREE.Mesh(new THREE.OctahedronGeometry(3.5,0),new THREE.MeshStandardMaterial({color:'#ff88cc',emissive:'#ff4499',emissiveIntensity:2.0,transparent:true,opacity:0.85,roughness:0.1,metalness:0.9}));
diamond.position.set(0,WH+pyrHeight,(OT+OBR)/2);s.add(diamond);
const dLight=new THREE.PointLight('#ff88cc',8,40,1.5);dLight.position.copy(diamond.position);s.add(dLight);
...
function addL(x,z){
  ...
  pl.position.set(x,WH-0.3,z);s.add(pl);pls.push({l:pl,base:5});
  // 圆形灯盘:与点光源的圆形光斑造型一致(原为长方形灯条,光形不符)
  const t=new THREE.Mesh(new THREE.CylinderGeometry(0.35,0.35,0.04,24),new THREE.MeshBasicMaterial({color:'#fffef5'}));
  t.position.set(x,WH-0.15,z);s.add(t);
}
// 展厅区灯光
for(let x=-14;x<=14;x+=7)for(let z=-9;z<=3;z+=6)addL(x,z);
addL(0,5);
// 回字北走廊灯光
addL(0,8);addL(-12,8);addL(12,8);
// 回字南走廊灯光
addL(0,25);addL(-12,25);addL(12,25);
// 回字西走廊灯光
addL(-12,17);
// 回字东走廊灯光
addL(12,17);
// 内墙门洞附近灯光
addL(0,10.5);addL(0,23.5);addL(-6.5,17);addL(6.5,17);
// 回字中央区域灯光
addL(0,17);addL(-3,14);addL(3,14);addL(-3,20);
// 环境灯光(存入变量供昼夜系统调节)
...
// 导出共享上下文
Object.assign(ctx.scene,{s,cam,rnd,L,jT,jB,aB,ray,mP2,iG,tL,loadTexCapped,OL,OR,OT,OBE,OBR,WH,IL,IR,IRT,IRB,bounds,floorW,floorD,bW,bD,pyrHeight,groundUniforms,skyUniforms,pls,ambL,hemiL});
