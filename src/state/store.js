// store.js — 存档深模块·冷核心(2026-07-28 架构深化②,方案 B)
...
function entry(name){
  const e=SCHEMA[name];
  if(!e)throw new Error('store: 未登记的存档键「'+name+'」(先去 store.js SCHEMA 登记)');
  return e;
}
...
ctx.ui.store={
  ...
  num(name){return +(rawGet(entry(name).key)||0);},
  setNum(name,v){rawSet(entry(name).key,String(v));},
  // —— 字符串:等价 getItem||def ——
  str(name,def){return rawGet(entry(name).key)||(def===undefined?'':def);},
  setStr(name,v){rawSet(entry(name).key,v);},
  ...
  setJson(name,v){rawSet(entry(name).key,JSON.stringify(v));},
  // —— 一次性标记:等价 !!getItem / setItem(k,'1') ——
  flag(name){return !!rawGet(entry(name).key);},
  mark(name){rawSet(entry(name).key,'1');},
  // —— 灵蕴库存(含旧档迁移,迁移逻辑从 spirits.js 收编于此) ——
  getSpirits(){
    let k=this.json('spiritsKeys',null);
    if(!Array.isArray(k)){ // 旧档:顺序收集时代只有数量键,前 n 颗即已集
  ...
  addSpirit(key){
    const k=this.getSpirits();k.push(key);
    rawSet(SCHEMA.spiritsKeys.key,JSON.stringify(k));
    rawSet(SCHEMA.spiritsCount.key,String(k.length)); // 兼容旧读取方(数量)
    return k;
  },
  // —— 房屋分组换色(动态键 houseColor_<组名>,仅自己可见) ——
  houseColor(g){return rawGet('houseColor_'+g)||'';},
  setHouseColor(g,hex){rawSet('houseColor_'+g,hex);},
  clearHouseColor(g){try{localStorage.removeItem('houseColor_'+g);}catch(e){}},
};
